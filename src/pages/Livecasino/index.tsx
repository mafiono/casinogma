import React from "react";

import { Button, Img, SelectBox, Text } from "components";
import CasinoSlots from "components/CasinoSlots";
import Header from "components/Header";
import Jackpot from "components/Jackpot";

const LivecasinoPage: React.FC = () => {
  return (
    <>
      <main className="main-rows">
        <div className="uc-row-wrapper">
          <div
            className="uc-row     first                       
                    
                                            "
            data-id={140}
            style={{
              "margin-bottom": "3.5169988276671vh",
              background: "transparent",
            }}
          >
            <div id="r140" className="row-container ">
              <div className="row mainRow " style={{}}>
                <div
                  id="column173"
                  className="column col-sm-12 "
                  datacolumn-id={173}
                  style={{ padding: "0vh 0vh" }}
                >
                  <div
                    className="column-container vertical-top"
                    style={{ "margin-top": "0vh" }}
                    aria-label="column container"
                    role="contentinfo"
                  >
                    <div
                      className="module-container   only-mobile   align-center ModuleSlider
                             first   last                             "
                      style={{ padding: "0px 0px 0px 0px" }}
                      aria-label="module container"
                      role="contentinfo"
                      id="m271"
                    >
                      <div
                        className="full-width    module ModuleSlider "
                        datamodule-id={271}
                        style={{ "max-width": "100%" }}
                      >
                        <div
                          id="slider271"
                          className="slider-wrapper  animated animated-271 "
                          data-animated-id={271}
                          style={{ "padding-top": "41.6979%" }}
                        >
                          <div className="slider-container  effect-block public-mode figure effect-default ">
                            <ul className="slider-container ">
                              <li
                                className="gallery-image slider-item active-slide"
                                style={{
                                  opacity: "1",
                                  "-webkit-transform": "translate(0px, 0px)",
                                  "-ms-transform": "translate(0px, 0px)",
                                  transform: "translate(0px, 0px)",
                                }}
                              >
                                <img
                                  className="cover-image image"
                                  style={{
                                    "object-position": "37.398% 37.475%",
                                  }}
                                  src="https://static.springbuilder.site/fs/userFiles-v2/betboard/images/271-cashback.webp?v=1646696293"
                                  loading="lazy"
                                  alt="271-cashback.png"
                                />
                                <div className="figcaption not-clickable"></div>
                              </li>
                            </ul>
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
        <div className="uc-row-wrapper">
          <div
            className="uc-row                          
                    
                                            "
            data-id={252}
            style={{ "margin-bottom": "0vh", background: "transparent" }}
          >
            <div id="r252" className="row-container ">
              <div className="row mainRow " style={{}}>
                <div
                  id="column301"
                  className="column col-sm-12 "
                  datacolumn-id={301}
                  style={{ background: "transparent" }}
                >
                  <div
                    className="column-container vertical-top"
                    style={{}}
                    aria-label="column container"
                    role="contentinfo"
                  >
                    <div
                      className="module-container   only-mobile   align-left ModuleWidgets
                             first   last                             "
                      style={{ padding: "0px 80px 0px 60px" }}
                      aria-label="module container"
                      role="contentinfo"
                      id="m499"
                    >
                      <div
                        className="   module ModuleWidgets "
                        datamodule-id={499}
                      >
                        <div
                          data-widget="Jackpot"
                          data-configs='{"fit":"cover","jackpotTitleFontSize":24,"jackpotPoolFontSize":56,"jackpotDetailsFontSize":12,"jackpotDetailsPoolFontSize":18,"jackpotBackground":false,"jackpotType":"0","moduleId":"499","type":"live","category":"sport","limit":5,"casinoGamesLimit":16}'
                          data-loaded="true"
                        >
                          <div className="Desktop__JackpotWrapper-sc-kop2kj-0 foprlK">
                            <div className="v3-carousel">
                              <div
                                className="slick-slider carousel__wrapper slick-initialized"
                                dir="ltr"
                              >
                                <div className="slick-list">
                                  <div
                                    className="slick-track"
                                    style={{ width: "1810px", opacity: "1" }}
                                  >
                                    <div
                                      data-index={0}
                                      className="slick-slide slick-active slick-current"
                                      tabIndex={-1}
                                      aria-hidden="false"
                                      style={{
                                        outline: "none",
                                        width: "362px",
                                        position: "relative",
                                        left: "0px",
                                        opacity: "1",
                                        "-webkit-transition":
                                          "opacity 500ms ease 0s, visibility 500ms ease 0s",
                                        transition:
                                          "opacity 500ms ease 0s, visibility 500ms ease 0s",
                                      }}
                                    >
                                      <div>
                                        <div className="jackpotCard__wrapper jackpotCard__wrapper--desktop">
                                          <div className="card">
                                            <div
                                              className="name"
                                              style={{ "font-size": "24px" }}
                                            >
                                              betconstruct jackpot
                                            </div>
                                            <div
                                              className="poolAmount"
                                              style={{ "font-size": "56px" }}
                                            >
                                              USD{" "}
                                              <div className="odometer odometer-auto-theme       odometer-animating-up odometer-animating">
                                                <div className="odometer-inside">
                                                  <span className="odometer-digit">
                                                    <span className="odometer-digit-spacer">
                                                      8
                                                    </span>
                                                    <span className="odometer-digit-inner">
                                                      <span className="odometer-ribbon">
                                                        <span className="odometer-ribbon-inner">
                                                          <div className="odometer-value odometer-last-value odometer-first-value">
                                                            1
                                                          </div>
                                                        </span>
                                                      </span>
                                                    </span>
                                                  </span>
                                                  <span className="odometer-formatting-mark">
                                                    ,
                                                  </span>
                                                  <span className="odometer-digit">
                                                    <span className="odometer-digit-spacer">
                                                      8
                                                    </span>
                                                    <span className="odometer-digit-inner">
                                                      <span className="odometer-ribbon">
                                                        <span className="odometer-ribbon-inner">
                                                          <div className="odometer-value odometer-last-value odometer-first-value">
                                                            0
                                                          </div>
                                                        </span>
                                                      </span>
                                                    </span>
                                                  </span>
                                                  <span className="odometer-digit">
                                                    <span className="odometer-digit-spacer">
                                                      8
                                                    </span>
                                                    <span className="odometer-digit-inner">
                                                      <span className="odometer-ribbon">
                                                        <span className="odometer-ribbon-inner">
                                                          <div className="odometer-value odometer-last-value odometer-first-value">
                                                            8
                                                          </div>
                                                        </span>
                                                      </span>
                                                    </span>
                                                  </span>
                                                  <span className="odometer-digit">
                                                    <span className="odometer-digit-spacer">
                                                      8
                                                    </span>
                                                    <span className="odometer-digit-inner">
                                                      <span className="odometer-ribbon">
                                                        <span className="odometer-ribbon-inner">
                                                          <div className="odometer-value odometer-last-value odometer-first-value">
                                                            3
                                                          </div>
                                                        </span>
                                                      </span>
                                                    </span>
                                                  </span>
                                                  <span className="odometer-formatting-mark">
                                                    ,
                                                  </span>
                                                  <span className="odometer-digit">
                                                    <span className="odometer-digit-spacer">
                                                      8
                                                    </span>
                                                    <span className="odometer-digit-inner">
                                                      <span className="odometer-ribbon">
                                                        <span className="odometer-ribbon-inner">
                                                          <div className="odometer-value odometer-last-value odometer-first-value">
                                                            3
                                                          </div>
                                                        </span>
                                                      </span>
                                                    </span>
                                                  </span>
                                                  <span className="odometer-digit">
                                                    <span className="odometer-digit-spacer">
                                                      8
                                                    </span>
                                                    <span className="odometer-digit-inner">
                                                      <span className="odometer-ribbon">
                                                        <span className="odometer-ribbon-inner">
                                                          <div className="odometer-value odometer-last-value odometer-first-value">
                                                            4
                                                          </div>
                                                        </span>
                                                      </span>
                                                    </span>
                                                  </span>
                                                  <span className="odometer-digit">
                                                    <span className="odometer-digit-spacer">
                                                      8
                                                    </span>
                                                    <span className="odometer-digit-inner">
                                                      <span className="odometer-ribbon">
                                                        <span className="odometer-ribbon-inner">
                                                          <div className="odometer-value odometer-first-value">
                                                            7
                                                          </div>
                                                          <div className="odometer-value odometer-last-value">
                                                            8
                                                          </div>
                                                        </span>
                                                      </span>
                                                    </span>
                                                  </span>
                                                  <span className="odometer-formatting-mark odometer-radix-mark">
                                                    .
                                                  </span>
                                                  <span className="odometer-digit">
                                                    <span className="odometer-digit-spacer">
                                                      8
                                                    </span>
                                                    <span className="odometer-digit-inner">
                                                      <span className="odometer-ribbon">
                                                        <span className="odometer-ribbon-inner">
                                                          <div className="odometer-value odometer-first-value">
                                                            2
                                                          </div>
                                                          <div className="odometer-value">
                                                            3
                                                          </div>
                                                          <div className="odometer-value">
                                                            4
                                                          </div>
                                                          <div className="odometer-value">
                                                            5
                                                          </div>
                                                          <div className="odometer-value">
                                                            6
                                                          </div>
                                                          <div className="odometer-value">
                                                            7
                                                          </div>
                                                          <div className="odometer-value">
                                                            8
                                                          </div>
                                                          <div className="odometer-value">
                                                            9
                                                          </div>
                                                          <div className="odometer-value">
                                                            0
                                                          </div>
                                                          <div className="odometer-value">
                                                            1
                                                          </div>
                                                          <div className="odometer-value">
                                                            2
                                                          </div>
                                                          <div className="odometer-value">
                                                            3
                                                          </div>
                                                          <div className="odometer-value">
                                                            4
                                                          </div>
                                                          <div className="odometer-value">
                                                            5
                                                          </div>
                                                          <div className="odometer-value odometer-last-value">
                                                            6
                                                          </div>
                                                        </span>
                                                      </span>
                                                    </span>
                                                  </span>
                                                  <span className="odometer-digit">
                                                    <span className="odometer-digit-spacer">
                                                      8
                                                    </span>
                                                    <span className="odometer-digit-inner">
                                                      <span className="odometer-ribbon">
                                                        <span className="odometer-ribbon-inner">
                                                          <div className="odometer-value odometer-first-value">
                                                            7
                                                          </div>
                                                          <div className="odometer-value">
                                                            0
                                                          </div>
                                                          <div className="odometer-value">
                                                            3
                                                          </div>
                                                          <div className="odometer-value">
                                                            6
                                                          </div>
                                                          <div className="odometer-value">
                                                            9
                                                          </div>
                                                          <div className="odometer-value">
                                                            2
                                                          </div>
                                                          <div className="odometer-value">
                                                            5
                                                          </div>
                                                          <div className="odometer-value">
                                                            8
                                                          </div>
                                                          <div className="odometer-value">
                                                            1
                                                          </div>
                                                          <div className="odometer-value">
                                                            4
                                                          </div>
                                                          <div className="odometer-value">
                                                            7
                                                          </div>
                                                          <div className="odometer-value">
                                                            0
                                                          </div>
                                                          <div className="odometer-value">
                                                            3
                                                          </div>
                                                          <div className="odometer-value">
                                                            6
                                                          </div>
                                                          <div className="odometer-value">
                                                            9
                                                          </div>
                                                          <div className="odometer-value">
                                                            2
                                                          </div>
                                                          <div className="odometer-value">
                                                            5
                                                          </div>
                                                          <div className="odometer-value">
                                                            8
                                                          </div>
                                                          <div className="odometer-value">
                                                            1
                                                          </div>
                                                          <div className="odometer-value">
                                                            4
                                                          </div>
                                                          <div className="odometer-value">
                                                            7
                                                          </div>
                                                          <div className="odometer-value">
                                                            0
                                                          </div>
                                                          <div className="odometer-value">
                                                            3
                                                          </div>
                                                          <div className="odometer-value">
                                                            7
                                                          </div>
                                                          <div className="odometer-value">
                                                            0
                                                          </div>
                                                          <div className="odometer-value">
                                                            3
                                                          </div>
                                                          <div className="odometer-value">
                                                            6
                                                          </div>
                                                          <div className="odometer-value">
                                                            9
                                                          </div>
                                                          <div className="odometer-value">
                                                            2
                                                          </div>
                                                          <div className="odometer-value">
                                                            5
                                                          </div>
                                                          <div className="odometer-value">
                                                            8
                                                          </div>
                                                          <div className="odometer-value">
                                                            1
                                                          </div>
                                                          <div className="odometer-value">
                                                            4
                                                          </div>
                                                          <div className="odometer-value">
                                                            7
                                                          </div>
                                                          <div className="odometer-value">
                                                            0
                                                          </div>
                                                          <div className="odometer-value">
                                                            3
                                                          </div>
                                                          <div className="odometer-value">
                                                            6
                                                          </div>
                                                          <div className="odometer-value">
                                                            9
                                                          </div>
                                                          <div className="odometer-value">
                                                            2
                                                          </div>
                                                          <div className="odometer-value">
                                                            5
                                                          </div>
                                                          <div className="odometer-value">
                                                            8
                                                          </div>
                                                          <div className="odometer-value">
                                                            1
                                                          </div>
                                                          <div className="odometer-value">
                                                            4
                                                          </div>
                                                          <div className="odometer-value">
                                                            7
                                                          </div>
                                                          <div className="odometer-value">
                                                            0
                                                          </div>
                                                          <div className="odometer-value odometer-last-value">
                                                            3
                                                          </div>
                                                        </span>
                                                      </span>
                                                    </span>
                                                  </span>
                                                </div>
                                              </div>
                                            </div>
                                          </div>
                                        </div>
                                      </div>
                                    </div>
                                    <div
                                      data-index={1}
                                      className="slick-slide"
                                      tabIndex={-1}
                                      aria-hidden="true"
                                      style={{
                                        outline: "none",
                                        width: "362px",
                                        position: "relative",
                                        left: "-362px",
                                        opacity: "0",
                                        "-webkit-transition":
                                          "opacity 500ms ease 0s, visibility 500ms ease 0s",
                                        transition:
                                          "opacity 500ms ease 0s, visibility 500ms ease 0s",
                                      }}
                                    >
                                      <div>
                                        <div className="jackpotCard__wrapper jackpotCard__wrapper--desktop">
                                          <div className="card">
                                            <div
                                              className="name"
                                              style={{ "font-size": "24px" }}
                                            >
                                              bettinggames jackpot
                                            </div>
                                            <div
                                              className="poolAmount"
                                              style={{ "font-size": "56px" }}
                                            >
                                              USD{" "}
                                              <div className="odometer odometer-auto-theme">
                                                <div className="odometer-inside">
                                                  <span className="odometer-digit">
                                                    <span className="odometer-digit-spacer">
                                                      8
                                                    </span>
                                                    <span className="odometer-digit-inner">
                                                      <span className="odometer-ribbon">
                                                        <span className="odometer-ribbon-inner">
                                                          <span className="odometer-value">
                                                            3
                                                          </span>
                                                        </span>
                                                      </span>
                                                    </span>
                                                  </span>
                                                  <span className="odometer-digit">
                                                    <span className="odometer-digit-spacer">
                                                      8
                                                    </span>
                                                    <span className="odometer-digit-inner">
                                                      <span className="odometer-ribbon">
                                                        <span className="odometer-ribbon-inner">
                                                          <span className="odometer-value">
                                                            8
                                                          </span>
                                                        </span>
                                                      </span>
                                                    </span>
                                                  </span>
                                                  <span className="odometer-formatting-mark">
                                                    ,
                                                  </span>
                                                  <span className="odometer-digit">
                                                    <span className="odometer-digit-spacer">
                                                      8
                                                    </span>
                                                    <span className="odometer-digit-inner">
                                                      <span className="odometer-ribbon">
                                                        <span className="odometer-ribbon-inner">
                                                          <span className="odometer-value">
                                                            0
                                                          </span>
                                                        </span>
                                                      </span>
                                                    </span>
                                                  </span>
                                                  <span className="odometer-digit">
                                                    <span className="odometer-digit-spacer">
                                                      8
                                                    </span>
                                                    <span className="odometer-digit-inner">
                                                      <span className="odometer-ribbon">
                                                        <span className="odometer-ribbon-inner">
                                                          <span className="odometer-value">
                                                            5
                                                          </span>
                                                        </span>
                                                      </span>
                                                    </span>
                                                  </span>
                                                  <span className="odometer-digit">
                                                    <span className="odometer-digit-spacer">
                                                      8
                                                    </span>
                                                    <span className="odometer-digit-inner">
                                                      <span className="odometer-ribbon">
                                                        <span className="odometer-ribbon-inner">
                                                          <span className="odometer-value">
                                                            8
                                                          </span>
                                                        </span>
                                                      </span>
                                                    </span>
                                                  </span>
                                                  <span className="odometer-formatting-mark odometer-radix-mark">
                                                    .
                                                  </span>
                                                  <span className="odometer-digit">
                                                    <span className="odometer-digit-spacer">
                                                      8
                                                    </span>
                                                    <span className="odometer-digit-inner">
                                                      <span className="odometer-ribbon">
                                                        <span className="odometer-ribbon-inner">
                                                          <span className="odometer-value">
                                                            0
                                                          </span>
                                                        </span>
                                                      </span>
                                                    </span>
                                                  </span>
                                                  <span className="odometer-digit">
                                                    <span className="odometer-digit-spacer">
                                                      8
                                                    </span>
                                                    <span className="odometer-digit-inner">
                                                      <span className="odometer-ribbon">
                                                        <span className="odometer-ribbon-inner">
                                                          <span className="odometer-value">
                                                            2
                                                          </span>
                                                        </span>
                                                      </span>
                                                    </span>
                                                  </span>
                                                </div>
                                              </div>
                                            </div>
                                          </div>
                                        </div>
                                      </div>
                                    </div>
                                  </div>
                                </div>
                                <ul
                                  className="slick-dots slick-dots-bottom"
                                  style={{ display: "block" }}
                                >
                                  <li className="slick-active">
                                    <button>1</button>
                                  </li>
                                  <li className>
                                    <button>2</button>
                                  </li>
                                </ul>
                              </div>
                            </div>
                            <div className="collapse__wrapper">
                              <div className="collapse__content">
                                <div className="collapse__header">
                                  <div className="title">See Details</div>
                                  <span
                                    className="v3-icon expand__icon"
                                    style={{
                                      "font-size": "8px",
                                      position: "relative",
                                    }}
                                  >
                                    <svg
                                      width="1em"
                                      height="1em"
                                      viewBox="0 0 32 32"
                                      fill="currentColor"
                                      xmlns="http://www.w3.org/2000/svg"
                                    >
                                      <path d="M15.9998 25.1417C15.7243 25.1412 15.4516 25.086 15.1975 24.9794C14.9435 24.8727 14.7131 24.7167 14.5198 24.5204L0.570476 10.4164C0.202148 10.0138 0.00212462 9.48527 0.011549 8.93964C0.0209734 8.39402 0.239129 7.87276 0.621142 7.48307C1.00272 7.09609 1.52075 6.87405 2.06414 6.86457C2.60752 6.8551 3.13298 7.05895 3.52781 7.4324L16.0078 20.0431L28.4878 7.4324C28.8826 7.05895 29.4081 6.8551 29.9515 6.86457C30.4949 6.87405 31.0129 7.09609 31.3945 7.48307C31.7765 7.87276 31.9946 8.39402 32.0041 8.93964C32.0135 9.48527 31.8135 10.0138 31.4451 10.4164L17.4878 24.5231C17.2933 24.7199 17.0616 24.8759 16.8061 24.9822C16.5506 25.0884 16.2765 25.1426 15.9998 25.1417V25.1417Z" />
                                    </svg>
                                  </span>
                                </div>
                              </div>
                            </div>
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
        <div className="uc-row-wrapper">
          <div
            className="uc-row                          
                    
                                            "
            data-id={308}
            style={{ background: "transparent" }}
          >
            <div
              id="r308"
              className="row-container "
              style={{ "z-index": "3" }}
            >
              <div className="row mainRow " style={{}}>
                <div
                  id="column387"
                  className="column col-xs-3 "
                  datacolumn-id={387}
                  style={{
                    padding: "1.053864168618267vh 0vh",
                    background: "transparent",
                  }}
                >
                  <div
                    className="column-container vertical-center"
                    style={{}}
                    aria-label="column container"
                    role="contentinfo"
                  >
                    <div
                      className="module-container   only-mobile   align-left ModuleWidgets
                             first   last                             "
                      style={{ padding: "0px 10px 0px 80px" }}
                      aria-label="module container"
                      role="contentinfo"
                      id="m620"
                    >
                      <div
                        className="   module ModuleWidgets "
                        datamodule-id={620}
                      >
                        <div
                          data-widget="CasinoProviders"
                          data-configs='{"fit":"cover","showLoadMoreBtn":true,"showGameTitleBlock":true,"bindWithCategories":false,"categoryIds":"28","showCategoryName":true,"moduleId":"620","type":"live","category":"sport","limit":5,"casinoGamesLimit":16}'
                          data-loaded="true"
                        >
                          <div id="casino-providers-block">
                            <div className="x-casinoProviders">
                              <div className="x-select">
                                <div
                                  className="v3-select v3-select-lg x-providersDropdown x-providersDropdown--allProvidersSelected v3-select-multiple v3-select-show-arrow"
                                  data-testid="providers_dropdown"
                                >
                                  <div className="v3-select-selector">
                                    <div className="v3-select-selection-overflow">
                                      <div
                                        className="v3-select-selection-overflow-item"
                                        style={{
                                          opacity: "0",
                                          "-webkit-order": "0",
                                          "-ms-flex-order": "0",
                                          order: "0",
                                          height: "0px",
                                          "overflow-y": "hidden",
                                          "pointer-events": "none",
                                          position: "absolute",
                                        }}
                                        aria-hidden="true"
                                      >
                                        <span
                                          className="v3-select-selection-item"
                                          title="All Providers"
                                        >
                                          <span className="v3-select-selection-item-content">
                                            <div className="x-providersDropdown__item">
                                              <span className="x-providersDropdown__item__firstBlock">
                                                All Providers
                                              </span>
                                              <div className="x-providersDropdown__item__secondBlock">
                                                <span className="x-providersDropdown__item__secondBlock__badge" />
                                              </div>
                                            </div>
                                          </span>
                                          <span
                                            className="v3-select-selection-item-remove"
                                            unselectable="on"
                                            aria-hidden="true"
                                            style={{
                                              "-webkit-user-select": "none",
                                              "-moz-user-select": "none",
                                              "-ms-user-select": "none",
                                              "user-select": "none",
                                            }}
                                          >
                                            <span
                                              role="img"
                                              aria-label="close"
                                              className="anticon anticon-close"
                                            >
                                              <svg
                                                viewBox="64 64 896 896"
                                                focusable="false"
                                                data-icon="close"
                                                width="1em"
                                                height="1em"
                                                fill="currentColor"
                                                aria-hidden="true"
                                              >
                                                <path d="M563.8 512l262.5-312.9c4.4-5.2.7-13.1-6.1-13.1h-79.8c-4.7 0-9.2 2.1-12.3 5.7L511.6 449.8 295.1 191.7c-3-3.6-7.5-5.7-12.3-5.7H203c-6.8 0-10.5 7.9-6.1 13.1L459.4 512 196.9 824.9A7.95 7.95 0 00203 838h79.8c4.7 0 9.2-2.1 12.3-5.7l216.5-258.1 216.5 258.1c3 3.6 7.5 5.7 12.3 5.7h79.8c6.8 0 10.5-7.9 6.1-13.1L563.8 512z" />
                                              </svg>
                                            </span>
                                          </span>
                                        </span>
                                      </div>
                                      <div
                                        className="v3-select-selection-overflow-item v3-select-selection-overflow-item-rest"
                                        style={{
                                          opacity: "1",
                                          "-webkit-order": "-1",
                                          "-ms-flex-order": "-1",
                                          order: "-1",
                                        }}
                                      >
                                        <span className="v3-select-selection-item">
                                          <span className="v3-select-selection-item-content">
                                            <span>+ 1</span>
                                          </span>
                                        </span>
                                      </div>
                                      <div
                                        className="v3-select-selection-overflow-item v3-select-selection-overflow-item-suffix"
                                        style={{
                                          opacity: "1",
                                          "-webkit-order": "-1",
                                          "-ms-flex-order": "-1",
                                          order: "-1",
                                        }}
                                      >
                                        <div
                                          className="v3-select-selection-search"
                                          style={{ width: "3px" }}
                                        >
                                          <input
                                            type="search"
                                            autoComplete="off"
                                            className="v3-select-selection-search-input"
                                            role="combobox"
                                            aria-haspopup="listbox"
                                            aria-owns="rc_select_0_list"
                                            aria-autocomplete="list"
                                            aria-controls="rc_select_0_list"
                                            aria-activedescendant="rc_select_0_list_0"
                                            defaultValue
                                            id="rc_select_0"
                                          />
                                          <span
                                            className="v3-select-selection-search-mirror"
                                            aria-hidden="true"
                                          >
                                            &nbsp;
                                          </span>
                                        </div>
                                      </div>
                                    </div>
                                  </div>
                                  <span
                                    className="v3-select-arrow"
                                    unselectable="on"
                                    aria-hidden="true"
                                    style={{
                                      "-webkit-user-select": "none",
                                      "-moz-user-select": "none",
                                      "-ms-user-select": "none",
                                      "user-select": "none",
                                    }}
                                  >
                                    <span
                                      className="v3-icon "
                                      style={{
                                        "font-size": "12px",
                                        position: "relative",
                                      }}
                                    >
                                      <svg
                                        width="1em"
                                        height="1em"
                                        viewBox="0 0 32 32"
                                        fill="currentColor"
                                        xmlns="http://www.w3.org/2000/svg"
                                      >
                                        <path d="M16.2667 24.16L2 9.89333L3.89333 8L16.2667 20.4L28.6667 8L30.56 9.89333L16.2667 24.16Z" />
                                      </svg>
                                    </span>
                                  </span>
                                </div>
                              </div>
                            </div>
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
                <div
                  id="column388"
                  className="column col-xs-3 "
                  datacolumn-id={388}
                  style={{ padding: "1.053864168618267vh 0vh" }}
                >
                  <div
                    className="column-container vertical-center"
                    style={{}}
                    aria-label="column container"
                    role="contentinfo"
                  >
                    <div
                      className="module-container   only-mobile   align-left ModuleWidgets
                             first   last                             "
                      style={{ padding: "0px 60px 0px 30px" }}
                      aria-label="module container"
                      role="contentinfo"
                      id="m621"
                    >
                      <div
                        className="   module ModuleWidgets "
                        datamodule-id={621}
                      >
                        <div
                          data-widget="Search"
                          data-configs='{"fit":"cover","searchType":"casino","bindWithCategories":false,"categoryIds":"","showCategoryName":true,"moduleId":"621","type":"live","category":"sport","limit":5,"casinoGamesLimit":16}'
                          data-loaded="true"
                        >
                          <div className="searchBar-container">
                            <div className="searchBar-wrapper searchBar-wrapper--absolute">
                              <div className="style__Wrapper-sc-qbxr52-0 hhMQjG searchBarWrapper searchWrap__desk">
                                <input
                                  autoComplete="off"
                                  placeholder="Search for game"
                                  id="search-input-bar"
                                  className="v3-input style__StyledInput-sc-qbxr52-1 bgbPnr"
                                  type="text"
                                  defaultValue
                                />
                                <span className="style__PostfixIconWrapper-sc-qbxr52-2 iTLza-d">
                                  <span
                                    className="v3-icon "
                                    style={{
                                      "font-size": "14px",
                                      position: "relative",
                                    }}
                                  >
                                    <svg
                                      width="1em"
                                      height="1em"
                                      viewBox="0 0 32 32"
                                      fill="currentColor"
                                      xmlns="http://www.w3.org/2000/svg"
                                    >
                                      <path d="M31.468 28.9577L23.62 21.1084C25.4034 18.7471 26.3368 15.8525 26.2686 12.8942C26.2005 9.93584 25.1349 7.08729 23.2447 4.81057C21.3545 2.53384 18.7506 0.962486 15.8553 0.351377C12.96 -0.259731 9.94311 0.125253 7.29405 1.44389C4.64499 2.76252 2.51911 4.93745 1.26125 7.6159C0.00338777 10.2944 -0.312681 13.3192 0.364312 16.1998C1.04131 19.0805 2.67165 21.6479 4.9909 23.4857C7.31015 25.3234 10.1823 26.3238 13.1414 26.3244C16.0106 26.3379 18.8029 25.397 21.0787 23.6497L28.9694 31.4564C29.1349 31.625 29.3328 31.7584 29.5511 31.8487C29.7694 31.939 30.0038 31.9842 30.24 31.9817C30.7159 31.9791 31.1719 31.7905 31.5107 31.4564C31.8319 31.1171 32.0074 30.6655 31.9994 30.1983C31.9914 29.7312 31.8007 29.2858 31.468 28.9577V28.9577ZM13.1894 22.7777C11.6123 22.7738 10.0606 22.3815 8.6712 21.6354C7.28182 20.8893 6.09765 19.8124 5.22335 18.5C4.34905 17.1875 3.81155 15.6798 3.65836 14.1103C3.50518 12.5407 3.74102 10.9576 4.34504 9.50079C4.94906 8.04403 5.90267 6.7585 7.12156 5.75785C8.34045 4.75719 9.78709 4.07222 11.3336 3.76349C12.8801 3.45476 14.4789 3.53178 15.9885 3.98773C17.4982 4.44368 18.8723 5.26453 19.9894 6.37771C20.8904 7.26386 21.6029 8.32307 22.0841 9.49165C22.5653 10.6602 22.8052 11.914 22.7894 13.1777C22.7924 15.7255 21.7856 18.1707 19.9894 19.9777C19.1008 20.8755 18.0411 21.5858 16.8731 22.0668C15.7051 22.5477 14.4525 22.7895 13.1894 22.7777V22.7777Z" />
                                    </svg>
                                  </span>
                                </span>
                              </div>
                              <div className="casino__wrapper-xs searchBar-wrapper--casino searchBar-wrapper--invisible searchBar-wrapper--loaded searchBar-wrapper--contain-casino" />
                            </div>
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
                <div
                  id="column389"
                  className="column col-xs-6 "
                  datacolumn-id={389}
                  style={{ padding: "1.053864168618267vh 0vh" }}
                >
                  <div
                    className="column-container vertical-top"
                    style={{}}
                    aria-label="column container"
                    role="contentinfo"
                  >
                    <div
                      className="module-container   only-mobile   align-center ModuleSpace
                             first   last                             "
                      style={{ padding: "0px 60px 0px 60px" }}
                      aria-label="module container"
                      role="contentinfo"
                      id="m622"
                    >
                      <div
                        className="full-width    module ModuleSpace "
                        datamodule-id={622}
                        style={{ "max-width": "100%" }}
                      >
                        <div className="space-container">
                          <div
                            className="space-inner"
                            style={{ height: "50px" }}
                          />
                        </div>
                        <style
                          dangerouslySetInnerHTML={{
                            __html:
                              '\n    .ModuleSpace[datamodule-id="622"] .space-container {\n                    border-radius: 0 0 0 0;\n                            }\n\n    .ModuleSpace[datamodule-id="622"]:hover .space-container{\n                    border-radius: 0 0 0 0;\n                        }\n',
                          }}
                        />
                        <style
                          dangerouslySetInnerHTML={{
                            __html:
                              '\n        .ModuleSpace[datamodule-id="622"] .uc-row .column:first-child {\n            border-bottom-left-radius: 0;\n            border-top-left-radius: 0;\n        }\n\n        .ModuleSpace[datamodule-id="622"] .uc-row .column:last-child {\n            border-bottom-right-radius: 0;\n            border-top-right-radius: 0;\n        }\n    ',
                          }}
                        />
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
        <div className="uc-row-wrapper">
          <div
            className="uc-row     last                       
                    
                                            "
            data-id={122}
            style={{
              "margin-bottom": "3.5169988276671vh",
              background: "transparent",
            }}
          >
            <div id="r122" className="row-container ">
              <div className="row mainRow " style={{}}>
                <div
                  id="column149"
                  className="column col-sm-12 "
                  datacolumn-id={149}
                  style={{}}
                >
                  <div
                    className="column-container vertical-top"
                    style={{}}
                    aria-label="column container"
                    role="contentinfo"
                  >
                    <div
                      className="module-container   only-mobile   align-right ModuleWidgets
                             first                              "
                      style={{ padding: "0px 80px 0px 80px" }}
                      aria-label="module container"
                      role="contentinfo"
                      id="m639"
                    >
                      <div
                        className="   module ModuleWidgets "
                        datamodule-id={639}
                      >
                        <div
                          data-widget="CasinoGames"
                          data-configs='{"fit":"cover","showLoadMoreBtn":true,"showGameTitleBlock":true,"bindWithCategories":false,"bindWithProviders":true,"loadMoreType":0,"groupAllByCategories":false,"gamesPerRow":6,"casinoNameSize":16,"casinoPlayRealBtnSize":44,"categoryIds":"28","showCategoryName":true,"moduleId":"639","type":"live","category":"sport","limit":5,"casinoGamesLimit":16}'
                          data-loaded="true"
                        >
                          <div className="x-casinoGames">
                            <div className="infinite-scroll-component__outerdiv">
                              <div
                                className="infinite-scroll-component "
                                style={{ height: "auto", overflow: "auto" }}
                              >
                                <div className="x-casinoGamesByCategories casino-container__70">
                                  <div
                                    className="x-casinoGamesByCategories__games"
                                    style={{ "--games-per-row": "6" }}
                                  >
                                    <div
                                      className="x-casinoGameCardDesktop"
                                      data-testid="casino_game_card"
                                    >
                                      <div className="x-casinoGameCardImageWrapper">
                                        <img
                                          className="x-casinoGameCardImageWrapper__image"
                                          src="https://cmsbetconstruct.com/content/images/casino/icon2/dfdd04a077c1a078fd908de312793209_casinoGameIcon2.webp?v=1686920992"
                                          alt="Auto Roulette"
                                          loading="lazy"
                                        />
                                        <div className="x-casinoGameCardDesktop__buttons">
                                          <div className="casino__real-btn">
                                            <div className="casino__real-btn-wrapper">
                                              <span
                                                className="v3-icon casino__real-btn-icon"
                                                style={{
                                                  "font-size": "44px",
                                                  position: "relative",
                                                }}
                                              >
                                                <svg
                                                  xmlns="http://www.w3.org/2000/svg"
                                                  width="1em"
                                                  height="1em"
                                                  viewBox="0 0 32 32"
                                                  fill="currentColor"
                                                >
                                                  <path d="M13.9013 23.4271C13.2464 23.4271 12.6213 23.2634 12.0706 22.9508C10.7906 22.2066 10.0762 20.748 10.0762 18.8429V13.8569C10.0762 11.9517 10.7757 10.4931 12.0557 9.74895C13.3357 9.00476 14.958 9.12383 16.6101 10.0764L20.9264 12.562C22.5785 13.5145 23.4864 14.8541 23.4864 16.3424C23.4864 17.8159 22.5785 19.1703 20.9264 20.1229L16.6101 22.6085C15.6873 23.1592 14.7497 23.4271 13.9013 23.4271ZM13.9013 11.4903C13.6334 11.4903 13.3804 11.5499 13.1869 11.6689C12.6213 11.9964 12.3087 12.7852 12.3087 13.8569V18.8429C12.3087 19.8996 12.6213 20.7034 13.1869 21.0159C13.7376 21.3434 14.5859 21.2094 15.5087 20.6885L19.825 18.2029C20.7478 17.6671 21.2687 16.9973 21.2687 16.3573C21.2687 15.7173 20.7329 15.0476 19.825 14.5117L15.5087 12.0262C14.9134 11.6689 14.3627 11.4903 13.9013 11.4903Z" />
                                                  <path d="M16 32C7.18884 32 0 24.826 0 16C0 7.17395 7.18884 0 16 0C24.8112 0 32 7.17395 32 16C32 24.826 24.826 32 16 32ZM16 2.23256C8.4093 2.23256 2.23256 8.4093 2.23256 16C2.23256 23.5907 8.4093 29.7674 16 29.7674C23.5907 29.7674 29.7674 23.5907 29.7674 16C29.7674 8.4093 23.5907 2.23256 16 2.23256Z" />
                                                </svg>
                                              </span>
                                              <span
                                                className="v3-icon casino__real-btn-icon-colored"
                                                style={{
                                                  "font-size": "44px",
                                                  position: "relative",
                                                }}
                                              >
                                                <svg
                                                  xmlns="http://www.w3.org/2000/svg"
                                                  width="1em"
                                                  height="1em"
                                                  viewBox="0 0 32 32"
                                                  fill="currentColor"
                                                >
                                                  <path d="M32 16C32 19.1645 31.0616 22.2579 29.3035 24.8891C27.5454 27.5203 25.0465 29.5711 22.1229 30.7821C19.1993 31.9931 15.9822 32.3099 12.8785 31.6926C9.77485 31.0752 6.92392 29.5513 4.68628 27.3137C2.44864 25.0761 0.924812 22.2251 0.307448 19.1214C-0.309915 16.0177 0.00691687 12.8007 1.21792 9.87706C2.42892 6.95344 4.47968 4.45459 7.11087 2.69648C9.74205 0.938383 12.8355 0 16 0C20.2426 0.00295546 24.3105 1.68961 27.3105 4.68955C30.3104 7.68949 31.997 11.7574 32 16Z" />
                                                  <path d="M23.2649 16.3677C23.2199 17.1494 22.9637 17.9045 22.5234 18.5533C22.0832 19.2021 20.7635 20.054 20.7635 20.054L16.5363 22.479C15.7382 22.9738 14.822 23.2479 13.8821 23.2728C13.2545 23.2724 12.6377 23.1114 12.0907 22.8052C11.7838 22.6243 11.5043 22.401 11.2606 22.1419C11.1394 22.0101 11.0297 21.8682 10.9328 21.7178C10.8254 21.5623 10.7305 21.3987 10.6489 21.2284C10.3962 20.6907 10.2399 20.2096 10.1901 19.5212C10.1711 19.2586 10.1901 19.1103 10.1901 18.847C10.1901 16.9488 10.1901 13.9863 10.1901 13.9863C10.1901 13.9863 10.1982 12.4035 10.5441 11.6908C10.89 10.978 11.4427 10.3849 12.1306 9.98806C12.8186 9.59118 13.6103 9.40877 14.4034 9.46437C15.1966 9.51998 16.58 10.3 16.58 10.3L20.8072 12.7249C21.5436 13.1246 22.1786 13.6862 22.6642 14.3669C22.9731 14.8037 23.1751 15.3065 23.254 15.8349C23.2743 16.0118 23.2779 16.1902 23.2649 16.3677Z" />
                                                </svg>
                                              </span>
                                            </div>
                                            <span
                                              className="casino__name"
                                              style={{ "font-size": "16px" }}
                                            >
                                              Auto Roulette
                                            </span>
                                            <span
                                              className="casino__provider-title"
                                              style={{ "font-size": "14px" }}
                                            >
                                              CREEDROOMZ
                                            </span>
                                          </div>
                                        </div>
                                        <div className="x-casinoGameCardDesktop__favoriteBtn">
                                          <span className="style__Container-sc-gq0272-0 iqWmzF">
                                            <span
                                              className="v3-icon "
                                              style={{
                                                "font-size": "16px",
                                                position: "relative",
                                              }}
                                            >
                                              <svg
                                                width="1em"
                                                height="1em"
                                                viewBox="0 0 32 32"
                                                fill="currentColor"
                                                xmlns="http://www.w3.org/2000/svg"
                                              >
                                                <path d="M22.8101 2.00043C21.4957 2.07648 20.2115 2.42514 19.0391 3.02424C17.8668 3.62335 16.8318 4.4598 16.0001 5.48043C15.1695 4.461 14.1362 3.62526 12.9656 3.0262C11.7951 2.42715 10.5128 2.07784 9.20013 2.00043C7.74063 1.9743 6.29765 2.31247 5.00173 2.98435C3.70582 3.65623 2.59787 4.64061 1.77813 5.84843C-4.40187 14.3484 7.29813 24.2904 10.0501 26.9604C11.6961 28.5604 13.7281 30.4544 14.9421 31.5784C15.2278 31.8492 15.6065 32.0002 16.0001 32.0002C16.3938 32.0002 16.7724 31.8492 17.0581 31.5784C18.2701 30.4544 20.3021 28.5584 21.9501 26.9604C24.7021 24.2904 36.4041 14.3484 30.2221 5.84843C29.4037 4.64165 28.2974 3.65787 27.0033 2.98602C25.7092 2.31417 24.268 1.97543 22.8101 2.00043V2.00043Z" />
                                              </svg>
                                            </span>
                                          </span>
                                        </div>
                                      </div>
                                    </div>
                                    <div
                                      className="x-casinoGameCardDesktop"
                                      data-testid="casino_game_card"
                                    >
                                      <div className="x-casinoGameCardImageWrapper">
                                        <img
                                          className="x-casinoGameCardImageWrapper__image"
                                          src="https://cmsbetconstruct.com/content/images/casino/icon2/2cfcd2b4a249381fb3e8e534fe41c15b_casinoGameIcon2.webp?v=1686920992"
                                          alt="Cashout Blackjack"
                                          loading="lazy"
                                        />
                                        <div className="x-casinoGameCardDesktop__buttons">
                                          <div className="casino__real-btn">
                                            <div className="casino__real-btn-wrapper">
                                              <span
                                                className="v3-icon casino__real-btn-icon"
                                                style={{
                                                  "font-size": "44px",
                                                  position: "relative",
                                                }}
                                              >
                                                <svg
                                                  xmlns="http://www.w3.org/2000/svg"
                                                  width="1em"
                                                  height="1em"
                                                  viewBox="0 0 32 32"
                                                  fill="currentColor"
                                                >
                                                  <path d="M13.9013 23.4271C13.2464 23.4271 12.6213 23.2634 12.0706 22.9508C10.7906 22.2066 10.0762 20.748 10.0762 18.8429V13.8569C10.0762 11.9517 10.7757 10.4931 12.0557 9.74895C13.3357 9.00476 14.958 9.12383 16.6101 10.0764L20.9264 12.562C22.5785 13.5145 23.4864 14.8541 23.4864 16.3424C23.4864 17.8159 22.5785 19.1703 20.9264 20.1229L16.6101 22.6085C15.6873 23.1592 14.7497 23.4271 13.9013 23.4271ZM13.9013 11.4903C13.6334 11.4903 13.3804 11.5499 13.1869 11.6689C12.6213 11.9964 12.3087 12.7852 12.3087 13.8569V18.8429C12.3087 19.8996 12.6213 20.7034 13.1869 21.0159C13.7376 21.3434 14.5859 21.2094 15.5087 20.6885L19.825 18.2029C20.7478 17.6671 21.2687 16.9973 21.2687 16.3573C21.2687 15.7173 20.7329 15.0476 19.825 14.5117L15.5087 12.0262C14.9134 11.6689 14.3627 11.4903 13.9013 11.4903Z" />
                                                  <path d="M16 32C7.18884 32 0 24.826 0 16C0 7.17395 7.18884 0 16 0C24.8112 0 32 7.17395 32 16C32 24.826 24.826 32 16 32ZM16 2.23256C8.4093 2.23256 2.23256 8.4093 2.23256 16C2.23256 23.5907 8.4093 29.7674 16 29.7674C23.5907 29.7674 29.7674 23.5907 29.7674 16C29.7674 8.4093 23.5907 2.23256 16 2.23256Z" />
                                                </svg>
                                              </span>
                                              <span
                                                className="v3-icon casino__real-btn-icon-colored"
                                                style={{
                                                  "font-size": "44px",
                                                  position: "relative",
                                                }}
                                              >
                                                <svg
                                                  xmlns="http://www.w3.org/2000/svg"
                                                  width="1em"
                                                  height="1em"
                                                  viewBox="0 0 32 32"
                                                  fill="currentColor"
                                                >
                                                  <path d="M32 16C32 19.1645 31.0616 22.2579 29.3035 24.8891C27.5454 27.5203 25.0465 29.5711 22.1229 30.7821C19.1993 31.9931 15.9822 32.3099 12.8785 31.6926C9.77485 31.0752 6.92392 29.5513 4.68628 27.3137C2.44864 25.0761 0.924812 22.2251 0.307448 19.1214C-0.309915 16.0177 0.00691687 12.8007 1.21792 9.87706C2.42892 6.95344 4.47968 4.45459 7.11087 2.69648C9.74205 0.938383 12.8355 0 16 0C20.2426 0.00295546 24.3105 1.68961 27.3105 4.68955C30.3104 7.68949 31.997 11.7574 32 16Z" />
                                                  <path d="M23.2649 16.3677C23.2199 17.1494 22.9637 17.9045 22.5234 18.5533C22.0832 19.2021 20.7635 20.054 20.7635 20.054L16.5363 22.479C15.7382 22.9738 14.822 23.2479 13.8821 23.2728C13.2545 23.2724 12.6377 23.1114 12.0907 22.8052C11.7838 22.6243 11.5043 22.401 11.2606 22.1419C11.1394 22.0101 11.0297 21.8682 10.9328 21.7178C10.8254 21.5623 10.7305 21.3987 10.6489 21.2284C10.3962 20.6907 10.2399 20.2096 10.1901 19.5212C10.1711 19.2586 10.1901 19.1103 10.1901 18.847C10.1901 16.9488 10.1901 13.9863 10.1901 13.9863C10.1901 13.9863 10.1982 12.4035 10.5441 11.6908C10.89 10.978 11.4427 10.3849 12.1306 9.98806C12.8186 9.59118 13.6103 9.40877 14.4034 9.46437C15.1966 9.51998 16.58 10.3 16.58 10.3L20.8072 12.7249C21.5436 13.1246 22.1786 13.6862 22.6642 14.3669C22.9731 14.8037 23.1751 15.3065 23.254 15.8349C23.2743 16.0118 23.2779 16.1902 23.2649 16.3677Z" />
                                                </svg>
                                              </span>
                                            </div>
                                            <span
                                              className="casino__name"
                                              style={{ "font-size": "16px" }}
                                            >
                                              Cashout Blackjack
                                            </span>
                                            <span
                                              className="casino__provider-title"
                                              style={{ "font-size": "14px" }}
                                            >
                                              CREEDROOMZ
                                            </span>
                                          </div>
                                        </div>
                                        <div className="x-casinoGameCardDesktop__favoriteBtn">
                                          <span className="style__Container-sc-gq0272-0 iqWmzF">
                                            <span
                                              className="v3-icon "
                                              style={{
                                                "font-size": "16px",
                                                position: "relative",
                                              }}
                                            >
                                              <svg
                                                width="1em"
                                                height="1em"
                                                viewBox="0 0 32 32"
                                                fill="currentColor"
                                                xmlns="http://www.w3.org/2000/svg"
                                              >
                                                <path d="M22.8101 2.00043C21.4957 2.07648 20.2115 2.42514 19.0391 3.02424C17.8668 3.62335 16.8318 4.4598 16.0001 5.48043C15.1695 4.461 14.1362 3.62526 12.9656 3.0262C11.7951 2.42715 10.5128 2.07784 9.20013 2.00043C7.74063 1.9743 6.29765 2.31247 5.00173 2.98435C3.70582 3.65623 2.59787 4.64061 1.77813 5.84843C-4.40187 14.3484 7.29813 24.2904 10.0501 26.9604C11.6961 28.5604 13.7281 30.4544 14.9421 31.5784C15.2278 31.8492 15.6065 32.0002 16.0001 32.0002C16.3938 32.0002 16.7724 31.8492 17.0581 31.5784C18.2701 30.4544 20.3021 28.5584 21.9501 26.9604C24.7021 24.2904 36.4041 14.3484 30.2221 5.84843C29.4037 4.64165 28.2974 3.65787 27.0033 2.98602C25.7092 2.31417 24.268 1.97543 22.8101 2.00043V2.00043Z" />
                                              </svg>
                                            </span>
                                          </span>
                                        </div>
                                      </div>
                                    </div>
                                    <div
                                      className="x-casinoGameCardDesktop"
                                      data-testid="casino_game_card"
                                    >
                                      <div className="x-casinoGameCardImageWrapper">
                                        <img
                                          className="x-casinoGameCardImageWrapper__image"
                                          src="https://cmsbetconstruct.com/content/images/casino/icon2/9ffcd548b8e866b4fcda1cd1501b168b_casinoGameIcon2.webp?v=1686920992"
                                          alt="Speed Blackjack"
                                          loading="lazy"
                                        />
                                        <div className="x-casinoGameCardDesktop__buttons">
                                          <div className="casino__real-btn">
                                            <div className="casino__real-btn-wrapper">
                                              <span
                                                className="v3-icon casino__real-btn-icon"
                                                style={{
                                                  "font-size": "44px",
                                                  position: "relative",
                                                }}
                                              >
                                                <svg
                                                  xmlns="http://www.w3.org/2000/svg"
                                                  width="1em"
                                                  height="1em"
                                                  viewBox="0 0 32 32"
                                                  fill="currentColor"
                                                >
                                                  <path d="M13.9013 23.4271C13.2464 23.4271 12.6213 23.2634 12.0706 22.9508C10.7906 22.2066 10.0762 20.748 10.0762 18.8429V13.8569C10.0762 11.9517 10.7757 10.4931 12.0557 9.74895C13.3357 9.00476 14.958 9.12383 16.6101 10.0764L20.9264 12.562C22.5785 13.5145 23.4864 14.8541 23.4864 16.3424C23.4864 17.8159 22.5785 19.1703 20.9264 20.1229L16.6101 22.6085C15.6873 23.1592 14.7497 23.4271 13.9013 23.4271ZM13.9013 11.4903C13.6334 11.4903 13.3804 11.5499 13.1869 11.6689C12.6213 11.9964 12.3087 12.7852 12.3087 13.8569V18.8429C12.3087 19.8996 12.6213 20.7034 13.1869 21.0159C13.7376 21.3434 14.5859 21.2094 15.5087 20.6885L19.825 18.2029C20.7478 17.6671 21.2687 16.9973 21.2687 16.3573C21.2687 15.7173 20.7329 15.0476 19.825 14.5117L15.5087 12.0262C14.9134 11.6689 14.3627 11.4903 13.9013 11.4903Z" />
                                                  <path d="M16 32C7.18884 32 0 24.826 0 16C0 7.17395 7.18884 0 16 0C24.8112 0 32 7.17395 32 16C32 24.826 24.826 32 16 32ZM16 2.23256C8.4093 2.23256 2.23256 8.4093 2.23256 16C2.23256 23.5907 8.4093 29.7674 16 29.7674C23.5907 29.7674 29.7674 23.5907 29.7674 16C29.7674 8.4093 23.5907 2.23256 16 2.23256Z" />
                                                </svg>
                                              </span>
                                              <span
                                                className="v3-icon casino__real-btn-icon-colored"
                                                style={{
                                                  "font-size": "44px",
                                                  position: "relative",
                                                }}
                                              >
                                                <svg
                                                  xmlns="http://www.w3.org/2000/svg"
                                                  width="1em"
                                                  height="1em"
                                                  viewBox="0 0 32 32"
                                                  fill="currentColor"
                                                >
                                                  <path d="M32 16C32 19.1645 31.0616 22.2579 29.3035 24.8891C27.5454 27.5203 25.0465 29.5711 22.1229 30.7821C19.1993 31.9931 15.9822 32.3099 12.8785 31.6926C9.77485 31.0752 6.92392 29.5513 4.68628 27.3137C2.44864 25.0761 0.924812 22.2251 0.307448 19.1214C-0.309915 16.0177 0.00691687 12.8007 1.21792 9.87706C2.42892 6.95344 4.47968 4.45459 7.11087 2.69648C9.74205 0.938383 12.8355 0 16 0C20.2426 0.00295546 24.3105 1.68961 27.3105 4.68955C30.3104 7.68949 31.997 11.7574 32 16Z" />
                                                  <path d="M23.2649 16.3677C23.2199 17.1494 22.9637 17.9045 22.5234 18.5533C22.0832 19.2021 20.7635 20.054 20.7635 20.054L16.5363 22.479C15.7382 22.9738 14.822 23.2479 13.8821 23.2728C13.2545 23.2724 12.6377 23.1114 12.0907 22.8052C11.7838 22.6243 11.5043 22.401 11.2606 22.1419C11.1394 22.0101 11.0297 21.8682 10.9328 21.7178C10.8254 21.5623 10.7305 21.3987 10.6489 21.2284C10.3962 20.6907 10.2399 20.2096 10.1901 19.5212C10.1711 19.2586 10.1901 19.1103 10.1901 18.847C10.1901 16.9488 10.1901 13.9863 10.1901 13.9863C10.1901 13.9863 10.1982 12.4035 10.5441 11.6908C10.89 10.978 11.4427 10.3849 12.1306 9.98806C12.8186 9.59118 13.6103 9.40877 14.4034 9.46437C15.1966 9.51998 16.58 10.3 16.58 10.3L20.8072 12.7249C21.5436 13.1246 22.1786 13.6862 22.6642 14.3669C22.9731 14.8037 23.1751 15.3065 23.254 15.8349C23.2743 16.0118 23.2779 16.1902 23.2649 16.3677Z" />
                                                </svg>
                                              </span>
                                            </div>
                                            <span
                                              className="casino__name"
                                              style={{ "font-size": "16px" }}
                                            >
                                              Speed Blackjack
                                            </span>
                                            <span
                                              className="casino__provider-title"
                                              style={{ "font-size": "14px" }}
                                            >
                                              CREEDROOMZ
                                            </span>
                                          </div>
                                        </div>
                                        <div className="x-casinoGameCardDesktop__favoriteBtn">
                                          <span className="style__Container-sc-gq0272-0 iqWmzF">
                                            <span
                                              className="v3-icon "
                                              style={{
                                                "font-size": "16px",
                                                position: "relative",
                                              }}
                                            >
                                              <svg
                                                width="1em"
                                                height="1em"
                                                viewBox="0 0 32 32"
                                                fill="currentColor"
                                                xmlns="http://www.w3.org/2000/svg"
                                              >
                                                <path d="M22.8101 2.00043C21.4957 2.07648 20.2115 2.42514 19.0391 3.02424C17.8668 3.62335 16.8318 4.4598 16.0001 5.48043C15.1695 4.461 14.1362 3.62526 12.9656 3.0262C11.7951 2.42715 10.5128 2.07784 9.20013 2.00043C7.74063 1.9743 6.29765 2.31247 5.00173 2.98435C3.70582 3.65623 2.59787 4.64061 1.77813 5.84843C-4.40187 14.3484 7.29813 24.2904 10.0501 26.9604C11.6961 28.5604 13.7281 30.4544 14.9421 31.5784C15.2278 31.8492 15.6065 32.0002 16.0001 32.0002C16.3938 32.0002 16.7724 31.8492 17.0581 31.5784C18.2701 30.4544 20.3021 28.5584 21.9501 26.9604C24.7021 24.2904 36.4041 14.3484 30.2221 5.84843C29.4037 4.64165 28.2974 3.65787 27.0033 2.98602C25.7092 2.31417 24.268 1.97543 22.8101 2.00043V2.00043Z" />
                                              </svg>
                                            </span>
                                          </span>
                                        </div>
                                      </div>
                                    </div>
                                    <div
                                      className="x-casinoGameCardDesktop"
                                      data-testid="casino_game_card"
                                    >
                                      <div className="x-casinoGameCardImageWrapper">
                                        <img
                                          className="x-casinoGameCardImageWrapper__image"
                                          src="https://cmsbetconstruct.com/content/images/casino/icon2/b3c65ae8f92f76d571f3291f5bc9bdac_casinoGameIcon2.webp?v=1686920992"
                                          alt="Two Hand Roba Baccarat"
                                          loading="lazy"
                                        />
                                        <div className="x-casinoGameCardDesktop__buttons">
                                          <div className="casino__real-btn">
                                            <div className="casino__real-btn-wrapper">
                                              <span
                                                className="v3-icon casino__real-btn-icon"
                                                style={{
                                                  "font-size": "44px",
                                                  position: "relative",
                                                }}
                                              >
                                                <svg
                                                  xmlns="http://www.w3.org/2000/svg"
                                                  width="1em"
                                                  height="1em"
                                                  viewBox="0 0 32 32"
                                                  fill="currentColor"
                                                >
                                                  <path d="M13.9013 23.4271C13.2464 23.4271 12.6213 23.2634 12.0706 22.9508C10.7906 22.2066 10.0762 20.748 10.0762 18.8429V13.8569C10.0762 11.9517 10.7757 10.4931 12.0557 9.74895C13.3357 9.00476 14.958 9.12383 16.6101 10.0764L20.9264 12.562C22.5785 13.5145 23.4864 14.8541 23.4864 16.3424C23.4864 17.8159 22.5785 19.1703 20.9264 20.1229L16.6101 22.6085C15.6873 23.1592 14.7497 23.4271 13.9013 23.4271ZM13.9013 11.4903C13.6334 11.4903 13.3804 11.5499 13.1869 11.6689C12.6213 11.9964 12.3087 12.7852 12.3087 13.8569V18.8429C12.3087 19.8996 12.6213 20.7034 13.1869 21.0159C13.7376 21.3434 14.5859 21.2094 15.5087 20.6885L19.825 18.2029C20.7478 17.6671 21.2687 16.9973 21.2687 16.3573C21.2687 15.7173 20.7329 15.0476 19.825 14.5117L15.5087 12.0262C14.9134 11.6689 14.3627 11.4903 13.9013 11.4903Z" />
                                                  <path d="M16 32C7.18884 32 0 24.826 0 16C0 7.17395 7.18884 0 16 0C24.8112 0 32 7.17395 32 16C32 24.826 24.826 32 16 32ZM16 2.23256C8.4093 2.23256 2.23256 8.4093 2.23256 16C2.23256 23.5907 8.4093 29.7674 16 29.7674C23.5907 29.7674 29.7674 23.5907 29.7674 16C29.7674 8.4093 23.5907 2.23256 16 2.23256Z" />
                                                </svg>
                                              </span>
                                              <span
                                                className="v3-icon casino__real-btn-icon-colored"
                                                style={{
                                                  "font-size": "44px",
                                                  position: "relative",
                                                }}
                                              >
                                                <svg
                                                  xmlns="http://www.w3.org/2000/svg"
                                                  width="1em"
                                                  height="1em"
                                                  viewBox="0 0 32 32"
                                                  fill="currentColor"
                                                >
                                                  <path d="M32 16C32 19.1645 31.0616 22.2579 29.3035 24.8891C27.5454 27.5203 25.0465 29.5711 22.1229 30.7821C19.1993 31.9931 15.9822 32.3099 12.8785 31.6926C9.77485 31.0752 6.92392 29.5513 4.68628 27.3137C2.44864 25.0761 0.924812 22.2251 0.307448 19.1214C-0.309915 16.0177 0.00691687 12.8007 1.21792 9.87706C2.42892 6.95344 4.47968 4.45459 7.11087 2.69648C9.74205 0.938383 12.8355 0 16 0C20.2426 0.00295546 24.3105 1.68961 27.3105 4.68955C30.3104 7.68949 31.997 11.7574 32 16Z" />
                                                  <path d="M23.2649 16.3677C23.2199 17.1494 22.9637 17.9045 22.5234 18.5533C22.0832 19.2021 20.7635 20.054 20.7635 20.054L16.5363 22.479C15.7382 22.9738 14.822 23.2479 13.8821 23.2728C13.2545 23.2724 12.6377 23.1114 12.0907 22.8052C11.7838 22.6243 11.5043 22.401 11.2606 22.1419C11.1394 22.0101 11.0297 21.8682 10.9328 21.7178C10.8254 21.5623 10.7305 21.3987 10.6489 21.2284C10.3962 20.6907 10.2399 20.2096 10.1901 19.5212C10.1711 19.2586 10.1901 19.1103 10.1901 18.847C10.1901 16.9488 10.1901 13.9863 10.1901 13.9863C10.1901 13.9863 10.1982 12.4035 10.5441 11.6908C10.89 10.978 11.4427 10.3849 12.1306 9.98806C12.8186 9.59118 13.6103 9.40877 14.4034 9.46437C15.1966 9.51998 16.58 10.3 16.58 10.3L20.8072 12.7249C21.5436 13.1246 22.1786 13.6862 22.6642 14.3669C22.9731 14.8037 23.1751 15.3065 23.254 15.8349C23.2743 16.0118 23.2779 16.1902 23.2649 16.3677Z" />
                                                </svg>
                                              </span>
                                            </div>
                                            <span
                                              className="casino__name"
                                              style={{ "font-size": "16px" }}
                                            >
                                              Two Hand Roba Baccarat
                                            </span>
                                            <span
                                              className="casino__provider-title"
                                              style={{ "font-size": "14px" }}
                                            >
                                              CREEDROOMZ
                                            </span>
                                          </div>
                                        </div>
                                        <div className="x-casinoGameCardDesktop__favoriteBtn">
                                          <span className="style__Container-sc-gq0272-0 iqWmzF">
                                            <span
                                              className="v3-icon "
                                              style={{
                                                "font-size": "16px",
                                                position: "relative",
                                              }}
                                            >
                                              <svg
                                                width="1em"
                                                height="1em"
                                                viewBox="0 0 32 32"
                                                fill="currentColor"
                                                xmlns="http://www.w3.org/2000/svg"
                                              >
                                                <path d="M22.8101 2.00043C21.4957 2.07648 20.2115 2.42514 19.0391 3.02424C17.8668 3.62335 16.8318 4.4598 16.0001 5.48043C15.1695 4.461 14.1362 3.62526 12.9656 3.0262C11.7951 2.42715 10.5128 2.07784 9.20013 2.00043C7.74063 1.9743 6.29765 2.31247 5.00173 2.98435C3.70582 3.65623 2.59787 4.64061 1.77813 5.84843C-4.40187 14.3484 7.29813 24.2904 10.0501 26.9604C11.6961 28.5604 13.7281 30.4544 14.9421 31.5784C15.2278 31.8492 15.6065 32.0002 16.0001 32.0002C16.3938 32.0002 16.7724 31.8492 17.0581 31.5784C18.2701 30.4544 20.3021 28.5584 21.9501 26.9604C24.7021 24.2904 36.4041 14.3484 30.2221 5.84843C29.4037 4.64165 28.2974 3.65787 27.0033 2.98602C25.7092 2.31417 24.268 1.97543 22.8101 2.00043V2.00043Z" />
                                              </svg>
                                            </span>
                                          </span>
                                        </div>
                                      </div>
                                    </div>
                                    <div
                                      className="x-casinoGameCardDesktop"
                                      data-testid="casino_game_card"
                                    >
                                      <div className="x-casinoGameCardImageWrapper">
                                        <img
                                          className="x-casinoGameCardImageWrapper__image"
                                          src="https://cmsbetconstruct.com/content/images/casino/icon2/cb5106cdda74048a552a69c3282d8329_casinoGameIcon2.webp?v=1686920992"
                                          alt="Russian Poker"
                                          loading="lazy"
                                        />
                                        <div className="x-casinoGameCardDesktop__buttons">
                                          <div className="casino__real-btn">
                                            <div className="casino__real-btn-wrapper">
                                              <span
                                                className="v3-icon casino__real-btn-icon"
                                                style={{
                                                  "font-size": "44px",
                                                  position: "relative",
                                                }}
                                              >
                                                <svg
                                                  xmlns="http://www.w3.org/2000/svg"
                                                  width="1em"
                                                  height="1em"
                                                  viewBox="0 0 32 32"
                                                  fill="currentColor"
                                                >
                                                  <path d="M13.9013 23.4271C13.2464 23.4271 12.6213 23.2634 12.0706 22.9508C10.7906 22.2066 10.0762 20.748 10.0762 18.8429V13.8569C10.0762 11.9517 10.7757 10.4931 12.0557 9.74895C13.3357 9.00476 14.958 9.12383 16.6101 10.0764L20.9264 12.562C22.5785 13.5145 23.4864 14.8541 23.4864 16.3424C23.4864 17.8159 22.5785 19.1703 20.9264 20.1229L16.6101 22.6085C15.6873 23.1592 14.7497 23.4271 13.9013 23.4271ZM13.9013 11.4903C13.6334 11.4903 13.3804 11.5499 13.1869 11.6689C12.6213 11.9964 12.3087 12.7852 12.3087 13.8569V18.8429C12.3087 19.8996 12.6213 20.7034 13.1869 21.0159C13.7376 21.3434 14.5859 21.2094 15.5087 20.6885L19.825 18.2029C20.7478 17.6671 21.2687 16.9973 21.2687 16.3573C21.2687 15.7173 20.7329 15.0476 19.825 14.5117L15.5087 12.0262C14.9134 11.6689 14.3627 11.4903 13.9013 11.4903Z" />
                                                  <path d="M16 32C7.18884 32 0 24.826 0 16C0 7.17395 7.18884 0 16 0C24.8112 0 32 7.17395 32 16C32 24.826 24.826 32 16 32ZM16 2.23256C8.4093 2.23256 2.23256 8.4093 2.23256 16C2.23256 23.5907 8.4093 29.7674 16 29.7674C23.5907 29.7674 29.7674 23.5907 29.7674 16C29.7674 8.4093 23.5907 2.23256 16 2.23256Z" />
                                                </svg>
                                              </span>
                                              <span
                                                className="v3-icon casino__real-btn-icon-colored"
                                                style={{
                                                  "font-size": "44px",
                                                  position: "relative",
                                                }}
                                              >
                                                <svg
                                                  xmlns="http://www.w3.org/2000/svg"
                                                  width="1em"
                                                  height="1em"
                                                  viewBox="0 0 32 32"
                                                  fill="currentColor"
                                                >
                                                  <path d="M32 16C32 19.1645 31.0616 22.2579 29.3035 24.8891C27.5454 27.5203 25.0465 29.5711 22.1229 30.7821C19.1993 31.9931 15.9822 32.3099 12.8785 31.6926C9.77485 31.0752 6.92392 29.5513 4.68628 27.3137C2.44864 25.0761 0.924812 22.2251 0.307448 19.1214C-0.309915 16.0177 0.00691687 12.8007 1.21792 9.87706C2.42892 6.95344 4.47968 4.45459 7.11087 2.69648C9.74205 0.938383 12.8355 0 16 0C20.2426 0.00295546 24.3105 1.68961 27.3105 4.68955C30.3104 7.68949 31.997 11.7574 32 16Z" />
                                                  <path d="M23.2649 16.3677C23.2199 17.1494 22.9637 17.9045 22.5234 18.5533C22.0832 19.2021 20.7635 20.054 20.7635 20.054L16.5363 22.479C15.7382 22.9738 14.822 23.2479 13.8821 23.2728C13.2545 23.2724 12.6377 23.1114 12.0907 22.8052C11.7838 22.6243 11.5043 22.401 11.2606 22.1419C11.1394 22.0101 11.0297 21.8682 10.9328 21.7178C10.8254 21.5623 10.7305 21.3987 10.6489 21.2284C10.3962 20.6907 10.2399 20.2096 10.1901 19.5212C10.1711 19.2586 10.1901 19.1103 10.1901 18.847C10.1901 16.9488 10.1901 13.9863 10.1901 13.9863C10.1901 13.9863 10.1982 12.4035 10.5441 11.6908C10.89 10.978 11.4427 10.3849 12.1306 9.98806C12.8186 9.59118 13.6103 9.40877 14.4034 9.46437C15.1966 9.51998 16.58 10.3 16.58 10.3L20.8072 12.7249C21.5436 13.1246 22.1786 13.6862 22.6642 14.3669C22.9731 14.8037 23.1751 15.3065 23.254 15.8349C23.2743 16.0118 23.2779 16.1902 23.2649 16.3677Z" />
                                                </svg>
                                              </span>
                                            </div>
                                            <span
                                              className="casino__name"
                                              style={{ "font-size": "16px" }}
                                            >
                                              Russian Poker
                                            </span>
                                            <span
                                              className="casino__provider-title"
                                              style={{ "font-size": "14px" }}
                                            >
                                              CREEDROOMZ
                                            </span>
                                          </div>
                                        </div>
                                        <div className="x-casinoGameCardDesktop__favoriteBtn">
                                          <span className="style__Container-sc-gq0272-0 iqWmzF">
                                            <span
                                              className="v3-icon "
                                              style={{
                                                "font-size": "16px",
                                                position: "relative",
                                              }}
                                            >
                                              <svg
                                                width="1em"
                                                height="1em"
                                                viewBox="0 0 32 32"
                                                fill="currentColor"
                                                xmlns="http://www.w3.org/2000/svg"
                                              >
                                                <path d="M22.8101 2.00043C21.4957 2.07648 20.2115 2.42514 19.0391 3.02424C17.8668 3.62335 16.8318 4.4598 16.0001 5.48043C15.1695 4.461 14.1362 3.62526 12.9656 3.0262C11.7951 2.42715 10.5128 2.07784 9.20013 2.00043C7.74063 1.9743 6.29765 2.31247 5.00173 2.98435C3.70582 3.65623 2.59787 4.64061 1.77813 5.84843C-4.40187 14.3484 7.29813 24.2904 10.0501 26.9604C11.6961 28.5604 13.7281 30.4544 14.9421 31.5784C15.2278 31.8492 15.6065 32.0002 16.0001 32.0002C16.3938 32.0002 16.7724 31.8492 17.0581 31.5784C18.2701 30.4544 20.3021 28.5584 21.9501 26.9604C24.7021 24.2904 36.4041 14.3484 30.2221 5.84843C29.4037 4.64165 28.2974 3.65787 27.0033 2.98602C25.7092 2.31417 24.268 1.97543 22.8101 2.00043V2.00043Z" />
                                              </svg>
                                            </span>
                                          </span>
                                        </div>
                                      </div>
                                    </div>
                                    <div
                                      className="x-casinoGameCardDesktop"
                                      data-testid="casino_game_card"
                                    >
                                      <div className="x-casinoGameCardImageWrapper">
                                        <img
                                          className="x-casinoGameCardImageWrapper__image"
                                          src="https://cmsbetconstruct.com/content/images/casino/icon2/5e64657075459279c730889ffd6097d3_casinoGameIcon2.webp?v=1686920992"
                                          alt="Sic Bo"
                                          loading="lazy"
                                        />
                                        <div className="x-casinoGameCardDesktop__buttons">
                                          <div className="casino__real-btn">
                                            <div className="casino__real-btn-wrapper">
                                              <span
                                                className="v3-icon casino__real-btn-icon"
                                                style={{
                                                  "font-size": "44px",
                                                  position: "relative",
                                                }}
                                              >
                                                <svg
                                                  xmlns="http://www.w3.org/2000/svg"
                                                  width="1em"
                                                  height="1em"
                                                  viewBox="0 0 32 32"
                                                  fill="currentColor"
                                                >
                                                  <path d="M13.9013 23.4271C13.2464 23.4271 12.6213 23.2634 12.0706 22.9508C10.7906 22.2066 10.0762 20.748 10.0762 18.8429V13.8569C10.0762 11.9517 10.7757 10.4931 12.0557 9.74895C13.3357 9.00476 14.958 9.12383 16.6101 10.0764L20.9264 12.562C22.5785 13.5145 23.4864 14.8541 23.4864 16.3424C23.4864 17.8159 22.5785 19.1703 20.9264 20.1229L16.6101 22.6085C15.6873 23.1592 14.7497 23.4271 13.9013 23.4271ZM13.9013 11.4903C13.6334 11.4903 13.3804 11.5499 13.1869 11.6689C12.6213 11.9964 12.3087 12.7852 12.3087 13.8569V18.8429C12.3087 19.8996 12.6213 20.7034 13.1869 21.0159C13.7376 21.3434 14.5859 21.2094 15.5087 20.6885L19.825 18.2029C20.7478 17.6671 21.2687 16.9973 21.2687 16.3573C21.2687 15.7173 20.7329 15.0476 19.825 14.5117L15.5087 12.0262C14.9134 11.6689 14.3627 11.4903 13.9013 11.4903Z" />
                                                  <path d="M16 32C7.18884 32 0 24.826 0 16C0 7.17395 7.18884 0 16 0C24.8112 0 32 7.17395 32 16C32 24.826 24.826 32 16 32ZM16 2.23256C8.4093 2.23256 2.23256 8.4093 2.23256 16C2.23256 23.5907 8.4093 29.7674 16 29.7674C23.5907 29.7674 29.7674 23.5907 29.7674 16C29.7674 8.4093 23.5907 2.23256 16 2.23256Z" />
                                                </svg>
                                              </span>
                                              <span
                                                className="v3-icon casino__real-btn-icon-colored"
                                                style={{
                                                  "font-size": "44px",
                                                  position: "relative",
                                                }}
                                              >
                                                <svg
                                                  xmlns="http://www.w3.org/2000/svg"
                                                  width="1em"
                                                  height="1em"
                                                  viewBox="0 0 32 32"
                                                  fill="currentColor"
                                                >
                                                  <path d="M32 16C32 19.1645 31.0616 22.2579 29.3035 24.8891C27.5454 27.5203 25.0465 29.5711 22.1229 30.7821C19.1993 31.9931 15.9822 32.3099 12.8785 31.6926C9.77485 31.0752 6.92392 29.5513 4.68628 27.3137C2.44864 25.0761 0.924812 22.2251 0.307448 19.1214C-0.309915 16.0177 0.00691687 12.8007 1.21792 9.87706C2.42892 6.95344 4.47968 4.45459 7.11087 2.69648C9.74205 0.938383 12.8355 0 16 0C20.2426 0.00295546 24.3105 1.68961 27.3105 4.68955C30.3104 7.68949 31.997 11.7574 32 16Z" />
                                                  <path d="M23.2649 16.3677C23.2199 17.1494 22.9637 17.9045 22.5234 18.5533C22.0832 19.2021 20.7635 20.054 20.7635 20.054L16.5363 22.479C15.7382 22.9738 14.822 23.2479 13.8821 23.2728C13.2545 23.2724 12.6377 23.1114 12.0907 22.8052C11.7838 22.6243 11.5043 22.401 11.2606 22.1419C11.1394 22.0101 11.0297 21.8682 10.9328 21.7178C10.8254 21.5623 10.7305 21.3987 10.6489 21.2284C10.3962 20.6907 10.2399 20.2096 10.1901 19.5212C10.1711 19.2586 10.1901 19.1103 10.1901 18.847C10.1901 16.9488 10.1901 13.9863 10.1901 13.9863C10.1901 13.9863 10.1982 12.4035 10.5441 11.6908C10.89 10.978 11.4427 10.3849 12.1306 9.98806C12.8186 9.59118 13.6103 9.40877 14.4034 9.46437C15.1966 9.51998 16.58 10.3 16.58 10.3L20.8072 12.7249C21.5436 13.1246 22.1786 13.6862 22.6642 14.3669C22.9731 14.8037 23.1751 15.3065 23.254 15.8349C23.2743 16.0118 23.2779 16.1902 23.2649 16.3677Z" />
                                                </svg>
                                              </span>
                                            </div>
                                            <span
                                              className="casino__name"
                                              style={{ "font-size": "16px" }}
                                            >
                                              Sic Bo
                                            </span>
                                            <span
                                              className="casino__provider-title"
                                              style={{ "font-size": "14px" }}
                                            >
                                              CREEDROOMZ
                                            </span>
                                          </div>
                                        </div>
                                        <div className="x-casinoGameCardDesktop__favoriteBtn">
                                          <span className="style__Container-sc-gq0272-0 iqWmzF">
                                            <span
                                              className="v3-icon "
                                              style={{
                                                "font-size": "16px",
                                                position: "relative",
                                              }}
                                            >
                                              <svg
                                                width="1em"
                                                height="1em"
                                                viewBox="0 0 32 32"
                                                fill="currentColor"
                                                xmlns="http://www.w3.org/2000/svg"
                                              >
                                                <path d="M22.8101 2.00043C21.4957 2.07648 20.2115 2.42514 19.0391 3.02424C17.8668 3.62335 16.8318 4.4598 16.0001 5.48043C15.1695 4.461 14.1362 3.62526 12.9656 3.0262C11.7951 2.42715 10.5128 2.07784 9.20013 2.00043C7.74063 1.9743 6.29765 2.31247 5.00173 2.98435C3.70582 3.65623 2.59787 4.64061 1.77813 5.84843C-4.40187 14.3484 7.29813 24.2904 10.0501 26.9604C11.6961 28.5604 13.7281 30.4544 14.9421 31.5784C15.2278 31.8492 15.6065 32.0002 16.0001 32.0002C16.3938 32.0002 16.7724 31.8492 17.0581 31.5784C18.2701 30.4544 20.3021 28.5584 21.9501 26.9604C24.7021 24.2904 36.4041 14.3484 30.2221 5.84843C29.4037 4.64165 28.2974 3.65787 27.0033 2.98602C25.7092 2.31417 24.268 1.97543 22.8101 2.00043V2.00043Z" />
                                              </svg>
                                            </span>
                                          </span>
                                        </div>
                                      </div>
                                    </div>
                                    <div
                                      className="x-casinoGameCardDesktop"
                                      data-testid="casino_game_card"
                                    >
                                      <div className="x-casinoGameCardImageWrapper">
                                        <img
                                          className="x-casinoGameCardImageWrapper__image"
                                          src="https://cmsbetconstruct.com/content/images/casino/icon2/31169bfdb3b9582ebb3a659b698ec19d_casinoGameIcon2.webp?v=1686920992"
                                          alt="VBET Blackjack"
                                          loading="lazy"
                                        />
                                        <div className="x-casinoGameCardDesktop__buttons">
                                          <div className="casino__real-btn">
                                            <div className="casino__real-btn-wrapper">
                                              <span
                                                className="v3-icon casino__real-btn-icon"
                                                style={{
                                                  "font-size": "44px",
                                                  position: "relative",
                                                }}
                                              >
                                                <svg
                                                  xmlns="http://www.w3.org/2000/svg"
                                                  width="1em"
                                                  height="1em"
                                                  viewBox="0 0 32 32"
                                                  fill="currentColor"
                                                >
                                                  <path d="M13.9013 23.4271C13.2464 23.4271 12.6213 23.2634 12.0706 22.9508C10.7906 22.2066 10.0762 20.748 10.0762 18.8429V13.8569C10.0762 11.9517 10.7757 10.4931 12.0557 9.74895C13.3357 9.00476 14.958 9.12383 16.6101 10.0764L20.9264 12.562C22.5785 13.5145 23.4864 14.8541 23.4864 16.3424C23.4864 17.8159 22.5785 19.1703 20.9264 20.1229L16.6101 22.6085C15.6873 23.1592 14.7497 23.4271 13.9013 23.4271ZM13.9013 11.4903C13.6334 11.4903 13.3804 11.5499 13.1869 11.6689C12.6213 11.9964 12.3087 12.7852 12.3087 13.8569V18.8429C12.3087 19.8996 12.6213 20.7034 13.1869 21.0159C13.7376 21.3434 14.5859 21.2094 15.5087 20.6885L19.825 18.2029C20.7478 17.6671 21.2687 16.9973 21.2687 16.3573C21.2687 15.7173 20.7329 15.0476 19.825 14.5117L15.5087 12.0262C14.9134 11.6689 14.3627 11.4903 13.9013 11.4903Z" />
                                                  <path d="M16 32C7.18884 32 0 24.826 0 16C0 7.17395 7.18884 0 16 0C24.8112 0 32 7.17395 32 16C32 24.826 24.826 32 16 32ZM16 2.23256C8.4093 2.23256 2.23256 8.4093 2.23256 16C2.23256 23.5907 8.4093 29.7674 16 29.7674C23.5907 29.7674 29.7674 23.5907 29.7674 16C29.7674 8.4093 23.5907 2.23256 16 2.23256Z" />
                                                </svg>
                                              </span>
                                              <span
                                                className="v3-icon casino__real-btn-icon-colored"
                                                style={{
                                                  "font-size": "44px",
                                                  position: "relative",
                                                }}
                                              >
                                                <svg
                                                  xmlns="http://www.w3.org/2000/svg"
                                                  width="1em"
                                                  height="1em"
                                                  viewBox="0 0 32 32"
                                                  fill="currentColor"
                                                >
                                                  <path d="M32 16C32 19.1645 31.0616 22.2579 29.3035 24.8891C27.5454 27.5203 25.0465 29.5711 22.1229 30.7821C19.1993 31.9931 15.9822 32.3099 12.8785 31.6926C9.77485 31.0752 6.92392 29.5513 4.68628 27.3137C2.44864 25.0761 0.924812 22.2251 0.307448 19.1214C-0.309915 16.0177 0.00691687 12.8007 1.21792 9.87706C2.42892 6.95344 4.47968 4.45459 7.11087 2.69648C9.74205 0.938383 12.8355 0 16 0C20.2426 0.00295546 24.3105 1.68961 27.3105 4.68955C30.3104 7.68949 31.997 11.7574 32 16Z" />
                                                  <path d="M23.2649 16.3677C23.2199 17.1494 22.9637 17.9045 22.5234 18.5533C22.0832 19.2021 20.7635 20.054 20.7635 20.054L16.5363 22.479C15.7382 22.9738 14.822 23.2479 13.8821 23.2728C13.2545 23.2724 12.6377 23.1114 12.0907 22.8052C11.7838 22.6243 11.5043 22.401 11.2606 22.1419C11.1394 22.0101 11.0297 21.8682 10.9328 21.7178C10.8254 21.5623 10.7305 21.3987 10.6489 21.2284C10.3962 20.6907 10.2399 20.2096 10.1901 19.5212C10.1711 19.2586 10.1901 19.1103 10.1901 18.847C10.1901 16.9488 10.1901 13.9863 10.1901 13.9863C10.1901 13.9863 10.1982 12.4035 10.5441 11.6908C10.89 10.978 11.4427 10.3849 12.1306 9.98806C12.8186 9.59118 13.6103 9.40877 14.4034 9.46437C15.1966 9.51998 16.58 10.3 16.58 10.3L20.8072 12.7249C21.5436 13.1246 22.1786 13.6862 22.6642 14.3669C22.9731 14.8037 23.1751 15.3065 23.254 15.8349C23.2743 16.0118 23.2779 16.1902 23.2649 16.3677Z" />
                                                </svg>
                                              </span>
                                            </div>
                                            <span
                                              className="casino__name"
                                              style={{ "font-size": "16px" }}
                                            >
                                              VBET Blackjack
                                            </span>
                                            <span
                                              className="casino__provider-title"
                                              style={{ "font-size": "14px" }}
                                            >
                                              Vbet
                                            </span>
                                          </div>
                                        </div>
                                        <div className="x-casinoGameCardDesktop__favoriteBtn">
                                          <span className="style__Container-sc-gq0272-0 iqWmzF">
                                            <span
                                              className="v3-icon "
                                              style={{
                                                "font-size": "16px",
                                                position: "relative",
                                              }}
                                            >
                                              <svg
                                                width="1em"
                                                height="1em"
                                                viewBox="0 0 32 32"
                                                fill="currentColor"
                                                xmlns="http://www.w3.org/2000/svg"
                                              >
                                                <path d="M22.8101 2.00043C21.4957 2.07648 20.2115 2.42514 19.0391 3.02424C17.8668 3.62335 16.8318 4.4598 16.0001 5.48043C15.1695 4.461 14.1362 3.62526 12.9656 3.0262C11.7951 2.42715 10.5128 2.07784 9.20013 2.00043C7.74063 1.9743 6.29765 2.31247 5.00173 2.98435C3.70582 3.65623 2.59787 4.64061 1.77813 5.84843C-4.40187 14.3484 7.29813 24.2904 10.0501 26.9604C11.6961 28.5604 13.7281 30.4544 14.9421 31.5784C15.2278 31.8492 15.6065 32.0002 16.0001 32.0002C16.3938 32.0002 16.7724 31.8492 17.0581 31.5784C18.2701 30.4544 20.3021 28.5584 21.9501 26.9604C24.7021 24.2904 36.4041 14.3484 30.2221 5.84843C29.4037 4.64165 28.2974 3.65787 27.0033 2.98602C25.7092 2.31417 24.268 1.97543 22.8101 2.00043V2.00043Z" />
                                              </svg>
                                            </span>
                                          </span>
                                        </div>
                                      </div>
                                    </div>
                                    <div
                                      className="x-casinoGameCardDesktop"
                                      data-testid="casino_game_card"
                                    >
                                      <div className="x-casinoGameCardImageWrapper">
                                        <img
                                          className="x-casinoGameCardImageWrapper__image"
                                          src="https://cmsbetconstruct.com/content/images/casino/icon2/ea2da38564b8e32b2bd6d7b7cc2893e6_casinoGameIcon2.gif?v=1686920992"
                                          alt="Diamond Fruits"
                                          loading="lazy"
                                        />
                                        <div className="x-casinoGameCardDesktop__buttons">
                                          <div className="casino__real-btn">
                                            <div className="casino__real-btn-wrapper">
                                              <span
                                                className="v3-icon casino__real-btn-icon"
                                                style={{
                                                  "font-size": "44px",
                                                  position: "relative",
                                                }}
                                              >
                                                <svg
                                                  xmlns="http://www.w3.org/2000/svg"
                                                  width="1em"
                                                  height="1em"
                                                  viewBox="0 0 32 32"
                                                  fill="currentColor"
                                                >
                                                  <path d="M13.9013 23.4271C13.2464 23.4271 12.6213 23.2634 12.0706 22.9508C10.7906 22.2066 10.0762 20.748 10.0762 18.8429V13.8569C10.0762 11.9517 10.7757 10.4931 12.0557 9.74895C13.3357 9.00476 14.958 9.12383 16.6101 10.0764L20.9264 12.562C22.5785 13.5145 23.4864 14.8541 23.4864 16.3424C23.4864 17.8159 22.5785 19.1703 20.9264 20.1229L16.6101 22.6085C15.6873 23.1592 14.7497 23.4271 13.9013 23.4271ZM13.9013 11.4903C13.6334 11.4903 13.3804 11.5499 13.1869 11.6689C12.6213 11.9964 12.3087 12.7852 12.3087 13.8569V18.8429C12.3087 19.8996 12.6213 20.7034 13.1869 21.0159C13.7376 21.3434 14.5859 21.2094 15.5087 20.6885L19.825 18.2029C20.7478 17.6671 21.2687 16.9973 21.2687 16.3573C21.2687 15.7173 20.7329 15.0476 19.825 14.5117L15.5087 12.0262C14.9134 11.6689 14.3627 11.4903 13.9013 11.4903Z" />
                                                  <path d="M16 32C7.18884 32 0 24.826 0 16C0 7.17395 7.18884 0 16 0C24.8112 0 32 7.17395 32 16C32 24.826 24.826 32 16 32ZM16 2.23256C8.4093 2.23256 2.23256 8.4093 2.23256 16C2.23256 23.5907 8.4093 29.7674 16 29.7674C23.5907 29.7674 29.7674 23.5907 29.7674 16C29.7674 8.4093 23.5907 2.23256 16 2.23256Z" />
                                                </svg>
                                              </span>
                                              <span
                                                className="v3-icon casino__real-btn-icon-colored"
                                                style={{
                                                  "font-size": "44px",
                                                  position: "relative",
                                                }}
                                              >
                                                <svg
                                                  xmlns="http://www.w3.org/2000/svg"
                                                  width="1em"
                                                  height="1em"
                                                  viewBox="0 0 32 32"
                                                  fill="currentColor"
                                                >
                                                  <path d="M32 16C32 19.1645 31.0616 22.2579 29.3035 24.8891C27.5454 27.5203 25.0465 29.5711 22.1229 30.7821C19.1993 31.9931 15.9822 32.3099 12.8785 31.6926C9.77485 31.0752 6.92392 29.5513 4.68628 27.3137C2.44864 25.0761 0.924812 22.2251 0.307448 19.1214C-0.309915 16.0177 0.00691687 12.8007 1.21792 9.87706C2.42892 6.95344 4.47968 4.45459 7.11087 2.69648C9.74205 0.938383 12.8355 0 16 0C20.2426 0.00295546 24.3105 1.68961 27.3105 4.68955C30.3104 7.68949 31.997 11.7574 32 16Z" />
                                                  <path d="M23.2649 16.3677C23.2199 17.1494 22.9637 17.9045 22.5234 18.5533C22.0832 19.2021 20.7635 20.054 20.7635 20.054L16.5363 22.479C15.7382 22.9738 14.822 23.2479 13.8821 23.2728C13.2545 23.2724 12.6377 23.1114 12.0907 22.8052C11.7838 22.6243 11.5043 22.401 11.2606 22.1419C11.1394 22.0101 11.0297 21.8682 10.9328 21.7178C10.8254 21.5623 10.7305 21.3987 10.6489 21.2284C10.3962 20.6907 10.2399 20.2096 10.1901 19.5212C10.1711 19.2586 10.1901 19.1103 10.1901 18.847C10.1901 16.9488 10.1901 13.9863 10.1901 13.9863C10.1901 13.9863 10.1982 12.4035 10.5441 11.6908C10.89 10.978 11.4427 10.3849 12.1306 9.98806C12.8186 9.59118 13.6103 9.40877 14.4034 9.46437C15.1966 9.51998 16.58 10.3 16.58 10.3L20.8072 12.7249C21.5436 13.1246 22.1786 13.6862 22.6642 14.3669C22.9731 14.8037 23.1751 15.3065 23.254 15.8349C23.2743 16.0118 23.2779 16.1902 23.2649 16.3677Z" />
                                                </svg>
                                              </span>
                                            </div>
                                            <span
                                              className="casino__name"
                                              style={{ "font-size": "16px" }}
                                            >
                                              Diamond Fruits
                                            </span>
                                            <span
                                              className="casino__provider-title"
                                              style={{ "font-size": "14px" }}
                                            >
                                              Popok Gaming
                                            </span>
                                          </div>
                                        </div>
                                        <div className="x-casinoGameCardDesktop__favoriteBtn">
                                          <span className="style__Container-sc-gq0272-0 iqWmzF">
                                            <span
                                              className="v3-icon "
                                              style={{
                                                "font-size": "16px",
                                                position: "relative",
                                              }}
                                            >
                                              <svg
                                                width="1em"
                                                height="1em"
                                                viewBox="0 0 32 32"
                                                fill="currentColor"
                                                xmlns="http://www.w3.org/2000/svg"
                                              >
                                                <path d="M22.8101 2.00043C21.4957 2.07648 20.2115 2.42514 19.0391 3.02424C17.8668 3.62335 16.8318 4.4598 16.0001 5.48043C15.1695 4.461 14.1362 3.62526 12.9656 3.0262C11.7951 2.42715 10.5128 2.07784 9.20013 2.00043C7.74063 1.9743 6.29765 2.31247 5.00173 2.98435C3.70582 3.65623 2.59787 4.64061 1.77813 5.84843C-4.40187 14.3484 7.29813 24.2904 10.0501 26.9604C11.6961 28.5604 13.7281 30.4544 14.9421 31.5784C15.2278 31.8492 15.6065 32.0002 16.0001 32.0002C16.3938 32.0002 16.7724 31.8492 17.0581 31.5784C18.2701 30.4544 20.3021 28.5584 21.9501 26.9604C24.7021 24.2904 36.4041 14.3484 30.2221 5.84843C29.4037 4.64165 28.2974 3.65787 27.0033 2.98602C25.7092 2.31417 24.268 1.97543 22.8101 2.00043V2.00043Z" />
                                              </svg>
                                            </span>
                                          </span>
                                        </div>
                                      </div>
                                    </div>
                                    <div
                                      className="x-casinoGameCardDesktop"
                                      data-testid="casino_game_card"
                                    >
                                      <div className="x-casinoGameCardImageWrapper">
                                        <img
                                          className="x-casinoGameCardImageWrapper__image"
                                          src="https://cmsbetconstruct.com/content/images/casino/icon2/04e0fec7177237a46583170f5a650fbd_casinoGameIcon2.webp?v=1686920992"
                                          alt="FTV Blackjack"
                                          loading="lazy"
                                        />
                                        <div className="x-casinoGameCardDesktop__buttons">
                                          <div className="casino__real-btn">
                                            <div className="casino__real-btn-wrapper">
                                              <span
                                                className="v3-icon casino__real-btn-icon"
                                                style={{
                                                  "font-size": "44px",
                                                  position: "relative",
                                                }}
                                              >
                                                <svg
                                                  xmlns="http://www.w3.org/2000/svg"
                                                  width="1em"
                                                  height="1em"
                                                  viewBox="0 0 32 32"
                                                  fill="currentColor"
                                                >
                                                  <path d="M13.9013 23.4271C13.2464 23.4271 12.6213 23.2634 12.0706 22.9508C10.7906 22.2066 10.0762 20.748 10.0762 18.8429V13.8569C10.0762 11.9517 10.7757 10.4931 12.0557 9.74895C13.3357 9.00476 14.958 9.12383 16.6101 10.0764L20.9264 12.562C22.5785 13.5145 23.4864 14.8541 23.4864 16.3424C23.4864 17.8159 22.5785 19.1703 20.9264 20.1229L16.6101 22.6085C15.6873 23.1592 14.7497 23.4271 13.9013 23.4271ZM13.9013 11.4903C13.6334 11.4903 13.3804 11.5499 13.1869 11.6689C12.6213 11.9964 12.3087 12.7852 12.3087 13.8569V18.8429C12.3087 19.8996 12.6213 20.7034 13.1869 21.0159C13.7376 21.3434 14.5859 21.2094 15.5087 20.6885L19.825 18.2029C20.7478 17.6671 21.2687 16.9973 21.2687 16.3573C21.2687 15.7173 20.7329 15.0476 19.825 14.5117L15.5087 12.0262C14.9134 11.6689 14.3627 11.4903 13.9013 11.4903Z" />
                                                  <path d="M16 32C7.18884 32 0 24.826 0 16C0 7.17395 7.18884 0 16 0C24.8112 0 32 7.17395 32 16C32 24.826 24.826 32 16 32ZM16 2.23256C8.4093 2.23256 2.23256 8.4093 2.23256 16C2.23256 23.5907 8.4093 29.7674 16 29.7674C23.5907 29.7674 29.7674 23.5907 29.7674 16C29.7674 8.4093 23.5907 2.23256 16 2.23256Z" />
                                                </svg>
                                              </span>
                                              <span
                                                className="v3-icon casino__real-btn-icon-colored"
                                                style={{
                                                  "font-size": "44px",
                                                  position: "relative",
                                                }}
                                              >
                                                <svg
                                                  xmlns="http://www.w3.org/2000/svg"
                                                  width="1em"
                                                  height="1em"
                                                  viewBox="0 0 32 32"
                                                  fill="currentColor"
                                                >
                                                  <path d="M32 16C32 19.1645 31.0616 22.2579 29.3035 24.8891C27.5454 27.5203 25.0465 29.5711 22.1229 30.7821C19.1993 31.9931 15.9822 32.3099 12.8785 31.6926C9.77485 31.0752 6.92392 29.5513 4.68628 27.3137C2.44864 25.0761 0.924812 22.2251 0.307448 19.1214C-0.309915 16.0177 0.00691687 12.8007 1.21792 9.87706C2.42892 6.95344 4.47968 4.45459 7.11087 2.69648C9.74205 0.938383 12.8355 0 16 0C20.2426 0.00295546 24.3105 1.68961 27.3105 4.68955C30.3104 7.68949 31.997 11.7574 32 16Z" />
                                                  <path d="M23.2649 16.3677C23.2199 17.1494 22.9637 17.9045 22.5234 18.5533C22.0832 19.2021 20.7635 20.054 20.7635 20.054L16.5363 22.479C15.7382 22.9738 14.822 23.2479 13.8821 23.2728C13.2545 23.2724 12.6377 23.1114 12.0907 22.8052C11.7838 22.6243 11.5043 22.401 11.2606 22.1419C11.1394 22.0101 11.0297 21.8682 10.9328 21.7178C10.8254 21.5623 10.7305 21.3987 10.6489 21.2284C10.3962 20.6907 10.2399 20.2096 10.1901 19.5212C10.1711 19.2586 10.1901 19.1103 10.1901 18.847C10.1901 16.9488 10.1901 13.9863 10.1901 13.9863C10.1901 13.9863 10.1982 12.4035 10.5441 11.6908C10.89 10.978 11.4427 10.3849 12.1306 9.98806C12.8186 9.59118 13.6103 9.40877 14.4034 9.46437C15.1966 9.51998 16.58 10.3 16.58 10.3L20.8072 12.7249C21.5436 13.1246 22.1786 13.6862 22.6642 14.3669C22.9731 14.8037 23.1751 15.3065 23.254 15.8349C23.2743 16.0118 23.2779 16.1902 23.2649 16.3677Z" />
                                                </svg>
                                              </span>
                                            </div>
                                            <span
                                              className="casino__name"
                                              style={{ "font-size": "16px" }}
                                            >
                                              FTV Blackjack
                                            </span>
                                            <span
                                              className="casino__provider-title"
                                              style={{ "font-size": "14px" }}
                                            >
                                              Fashion TV
                                            </span>
                                          </div>
                                        </div>
                                        <div className="x-casinoGameCardDesktop__favoriteBtn">
                                          <span className="style__Container-sc-gq0272-0 iqWmzF">
                                            <span
                                              className="v3-icon "
                                              style={{
                                                "font-size": "16px",
                                                position: "relative",
                                              }}
                                            >
                                              <svg
                                                width="1em"
                                                height="1em"
                                                viewBox="0 0 32 32"
                                                fill="currentColor"
                                                xmlns="http://www.w3.org/2000/svg"
                                              >
                                                <path d="M22.8101 2.00043C21.4957 2.07648 20.2115 2.42514 19.0391 3.02424C17.8668 3.62335 16.8318 4.4598 16.0001 5.48043C15.1695 4.461 14.1362 3.62526 12.9656 3.0262C11.7951 2.42715 10.5128 2.07784 9.20013 2.00043C7.74063 1.9743 6.29765 2.31247 5.00173 2.98435C3.70582 3.65623 2.59787 4.64061 1.77813 5.84843C-4.40187 14.3484 7.29813 24.2904 10.0501 26.9604C11.6961 28.5604 13.7281 30.4544 14.9421 31.5784C15.2278 31.8492 15.6065 32.0002 16.0001 32.0002C16.3938 32.0002 16.7724 31.8492 17.0581 31.5784C18.2701 30.4544 20.3021 28.5584 21.9501 26.9604C24.7021 24.2904 36.4041 14.3484 30.2221 5.84843C29.4037 4.64165 28.2974 3.65787 27.0033 2.98602C25.7092 2.31417 24.268 1.97543 22.8101 2.00043V2.00043Z" />
                                              </svg>
                                            </span>
                                          </span>
                                        </div>
                                      </div>
                                    </div>
                                    <div
                                      className="x-casinoGameCardDesktop"
                                      data-testid="casino_game_card"
                                    >
                                      <div className="x-casinoGameCardImageWrapper">
                                        <img
                                          className="x-casinoGameCardImageWrapper__image"
                                          src="https://cmsbetconstruct.com/content/images/casino/icon2/f5f2fe4a1fbcda2de64d1b9381e2ea13_casinoGameIcon2.webp?v=1686920992"
                                          alt="Richie Roulette"
                                          loading="lazy"
                                        />
                                        <div className="x-casinoGameCardDesktop__buttons">
                                          <div className="casino__real-btn">
                                            <div className="casino__real-btn-wrapper">
                                              <span
                                                className="v3-icon casino__real-btn-icon"
                                                style={{
                                                  "font-size": "44px",
                                                  position: "relative",
                                                }}
                                              >
                                                <svg
                                                  xmlns="http://www.w3.org/2000/svg"
                                                  width="1em"
                                                  height="1em"
                                                  viewBox="0 0 32 32"
                                                  fill="currentColor"
                                                >
                                                  <path d="M13.9013 23.4271C13.2464 23.4271 12.6213 23.2634 12.0706 22.9508C10.7906 22.2066 10.0762 20.748 10.0762 18.8429V13.8569C10.0762 11.9517 10.7757 10.4931 12.0557 9.74895C13.3357 9.00476 14.958 9.12383 16.6101 10.0764L20.9264 12.562C22.5785 13.5145 23.4864 14.8541 23.4864 16.3424C23.4864 17.8159 22.5785 19.1703 20.9264 20.1229L16.6101 22.6085C15.6873 23.1592 14.7497 23.4271 13.9013 23.4271ZM13.9013 11.4903C13.6334 11.4903 13.3804 11.5499 13.1869 11.6689C12.6213 11.9964 12.3087 12.7852 12.3087 13.8569V18.8429C12.3087 19.8996 12.6213 20.7034 13.1869 21.0159C13.7376 21.3434 14.5859 21.2094 15.5087 20.6885L19.825 18.2029C20.7478 17.6671 21.2687 16.9973 21.2687 16.3573C21.2687 15.7173 20.7329 15.0476 19.825 14.5117L15.5087 12.0262C14.9134 11.6689 14.3627 11.4903 13.9013 11.4903Z" />
                                                  <path d="M16 32C7.18884 32 0 24.826 0 16C0 7.17395 7.18884 0 16 0C24.8112 0 32 7.17395 32 16C32 24.826 24.826 32 16 32ZM16 2.23256C8.4093 2.23256 2.23256 8.4093 2.23256 16C2.23256 23.5907 8.4093 29.7674 16 29.7674C23.5907 29.7674 29.7674 23.5907 29.7674 16C29.7674 8.4093 23.5907 2.23256 16 2.23256Z" />
                                                </svg>
                                              </span>
                                              <span
                                                className="v3-icon casino__real-btn-icon-colored"
                                                style={{
                                                  "font-size": "44px",
                                                  position: "relative",
                                                }}
                                              >
                                                <svg
                                                  xmlns="http://www.w3.org/2000/svg"
                                                  width="1em"
                                                  height="1em"
                                                  viewBox="0 0 32 32"
                                                  fill="currentColor"
                                                >
                                                  <path d="M32 16C32 19.1645 31.0616 22.2579 29.3035 24.8891C27.5454 27.5203 25.0465 29.5711 22.1229 30.7821C19.1993 31.9931 15.9822 32.3099 12.8785 31.6926C9.77485 31.0752 6.92392 29.5513 4.68628 27.3137C2.44864 25.0761 0.924812 22.2251 0.307448 19.1214C-0.309915 16.0177 0.00691687 12.8007 1.21792 9.87706C2.42892 6.95344 4.47968 4.45459 7.11087 2.69648C9.74205 0.938383 12.8355 0 16 0C20.2426 0.00295546 24.3105 1.68961 27.3105 4.68955C30.3104 7.68949 31.997 11.7574 32 16Z" />
                                                  <path d="M23.2649 16.3677C23.2199 17.1494 22.9637 17.9045 22.5234 18.5533C22.0832 19.2021 20.7635 20.054 20.7635 20.054L16.5363 22.479C15.7382 22.9738 14.822 23.2479 13.8821 23.2728C13.2545 23.2724 12.6377 23.1114 12.0907 22.8052C11.7838 22.6243 11.5043 22.401 11.2606 22.1419C11.1394 22.0101 11.0297 21.8682 10.9328 21.7178C10.8254 21.5623 10.7305 21.3987 10.6489 21.2284C10.3962 20.6907 10.2399 20.2096 10.1901 19.5212C10.1711 19.2586 10.1901 19.1103 10.1901 18.847C10.1901 16.9488 10.1901 13.9863 10.1901 13.9863C10.1901 13.9863 10.1982 12.4035 10.5441 11.6908C10.89 10.978 11.4427 10.3849 12.1306 9.98806C12.8186 9.59118 13.6103 9.40877 14.4034 9.46437C15.1966 9.51998 16.58 10.3 16.58 10.3L20.8072 12.7249C21.5436 13.1246 22.1786 13.6862 22.6642 14.3669C22.9731 14.8037 23.1751 15.3065 23.254 15.8349C23.2743 16.0118 23.2779 16.1902 23.2649 16.3677Z" />
                                                </svg>
                                              </span>
                                            </div>
                                            <span
                                              className="casino__name"
                                              style={{ "font-size": "16px" }}
                                            >
                                              Richie Roulette
                                            </span>
                                            <span
                                              className="casino__provider-title"
                                              style={{ "font-size": "14px" }}
                                            >
                                              CREEDROOMZ
                                            </span>
                                          </div>
                                        </div>
                                        <div className="x-casinoGameCardDesktop__favoriteBtn">
                                          <span className="style__Container-sc-gq0272-0 iqWmzF">
                                            <span
                                              className="v3-icon "
                                              style={{
                                                "font-size": "16px",
                                                position: "relative",
                                              }}
                                            >
                                              <svg
                                                width="1em"
                                                height="1em"
                                                viewBox="0 0 32 32"
                                                fill="currentColor"
                                                xmlns="http://www.w3.org/2000/svg"
                                              >
                                                <path d="M22.8101 2.00043C21.4957 2.07648 20.2115 2.42514 19.0391 3.02424C17.8668 3.62335 16.8318 4.4598 16.0001 5.48043C15.1695 4.461 14.1362 3.62526 12.9656 3.0262C11.7951 2.42715 10.5128 2.07784 9.20013 2.00043C7.74063 1.9743 6.29765 2.31247 5.00173 2.98435C3.70582 3.65623 2.59787 4.64061 1.77813 5.84843C-4.40187 14.3484 7.29813 24.2904 10.0501 26.9604C11.6961 28.5604 13.7281 30.4544 14.9421 31.5784C15.2278 31.8492 15.6065 32.0002 16.0001 32.0002C16.3938 32.0002 16.7724 31.8492 17.0581 31.5784C18.2701 30.4544 20.3021 28.5584 21.9501 26.9604C24.7021 24.2904 36.4041 14.3484 30.2221 5.84843C29.4037 4.64165 28.2974 3.65787 27.0033 2.98602C25.7092 2.31417 24.268 1.97543 22.8101 2.00043V2.00043Z" />
                                              </svg>
                                            </span>
                                          </span>
                                        </div>
                                      </div>
                                    </div>
                                    <div
                                      className="x-casinoGameCardDesktop"
                                      data-testid="casino_game_card"
                                    >
                                      <div className="x-casinoGameCardImageWrapper">
                                        <img
                                          className="x-casinoGameCardImageWrapper__image"
                                          src="https://cmsbetconstruct.com/content/images/casino/icon2/106fdae8330d1b35963a482780bb5ef7_casinoGameIcon2.webp?v=1686920992"
                                          alt="ROBA Baccarat"
                                          loading="lazy"
                                        />
                                        <div className="x-casinoGameCardDesktop__buttons">
                                          <div className="casino__real-btn">
                                            <div className="casino__real-btn-wrapper">
                                              <span
                                                className="v3-icon casino__real-btn-icon"
                                                style={{
                                                  "font-size": "44px",
                                                  position: "relative",
                                                }}
                                              >
                                                <svg
                                                  xmlns="http://www.w3.org/2000/svg"
                                                  width="1em"
                                                  height="1em"
                                                  viewBox="0 0 32 32"
                                                  fill="currentColor"
                                                >
                                                  <path d="M13.9013 23.4271C13.2464 23.4271 12.6213 23.2634 12.0706 22.9508C10.7906 22.2066 10.0762 20.748 10.0762 18.8429V13.8569C10.0762 11.9517 10.7757 10.4931 12.0557 9.74895C13.3357 9.00476 14.958 9.12383 16.6101 10.0764L20.9264 12.562C22.5785 13.5145 23.4864 14.8541 23.4864 16.3424C23.4864 17.8159 22.5785 19.1703 20.9264 20.1229L16.6101 22.6085C15.6873 23.1592 14.7497 23.4271 13.9013 23.4271ZM13.9013 11.4903C13.6334 11.4903 13.3804 11.5499 13.1869 11.6689C12.6213 11.9964 12.3087 12.7852 12.3087 13.8569V18.8429C12.3087 19.8996 12.6213 20.7034 13.1869 21.0159C13.7376 21.3434 14.5859 21.2094 15.5087 20.6885L19.825 18.2029C20.7478 17.6671 21.2687 16.9973 21.2687 16.3573C21.2687 15.7173 20.7329 15.0476 19.825 14.5117L15.5087 12.0262C14.9134 11.6689 14.3627 11.4903 13.9013 11.4903Z" />
                                                  <path d="M16 32C7.18884 32 0 24.826 0 16C0 7.17395 7.18884 0 16 0C24.8112 0 32 7.17395 32 16C32 24.826 24.826 32 16 32ZM16 2.23256C8.4093 2.23256 2.23256 8.4093 2.23256 16C2.23256 23.5907 8.4093 29.7674 16 29.7674C23.5907 29.7674 29.7674 23.5907 29.7674 16C29.7674 8.4093 23.5907 2.23256 16 2.23256Z" />
                                                </svg>
                                              </span>
                                              <span
                                                className="v3-icon casino__real-btn-icon-colored"
                                                style={{
                                                  "font-size": "44px",
                                                  position: "relative",
                                                }}
                                              >
                                                <svg
                                                  xmlns="http://www.w3.org/2000/svg"
                                                  width="1em"
                                                  height="1em"
                                                  viewBox="0 0 32 32"
                                                  fill="currentColor"
                                                >
                                                  <path d="M32 16C32 19.1645 31.0616 22.2579 29.3035 24.8891C27.5454 27.5203 25.0465 29.5711 22.1229 30.7821C19.1993 31.9931 15.9822 32.3099 12.8785 31.6926C9.77485 31.0752 6.92392 29.5513 4.68628 27.3137C2.44864 25.0761 0.924812 22.2251 0.307448 19.1214C-0.309915 16.0177 0.00691687 12.8007 1.21792 9.87706C2.42892 6.95344 4.47968 4.45459 7.11087 2.69648C9.74205 0.938383 12.8355 0 16 0C20.2426 0.00295546 24.3105 1.68961 27.3105 4.68955C30.3104 7.68949 31.997 11.7574 32 16Z" />
                                                  <path d="M23.2649 16.3677C23.2199 17.1494 22.9637 17.9045 22.5234 18.5533C22.0832 19.2021 20.7635 20.054 20.7635 20.054L16.5363 22.479C15.7382 22.9738 14.822 23.2479 13.8821 23.2728C13.2545 23.2724 12.6377 23.1114 12.0907 22.8052C11.7838 22.6243 11.5043 22.401 11.2606 22.1419C11.1394 22.0101 11.0297 21.8682 10.9328 21.7178C10.8254 21.5623 10.7305 21.3987 10.6489 21.2284C10.3962 20.6907 10.2399 20.2096 10.1901 19.5212C10.1711 19.2586 10.1901 19.1103 10.1901 18.847C10.1901 16.9488 10.1901 13.9863 10.1901 13.9863C10.1901 13.9863 10.1982 12.4035 10.5441 11.6908C10.89 10.978 11.4427 10.3849 12.1306 9.98806C12.8186 9.59118 13.6103 9.40877 14.4034 9.46437C15.1966 9.51998 16.58 10.3 16.58 10.3L20.8072 12.7249C21.5436 13.1246 22.1786 13.6862 22.6642 14.3669C22.9731 14.8037 23.1751 15.3065 23.254 15.8349C23.2743 16.0118 23.2779 16.1902 23.2649 16.3677Z" />
                                                </svg>
                                              </span>
                                            </div>
                                            <span
                                              className="casino__name"
                                              style={{ "font-size": "16px" }}
                                            >
                                              ROBA Baccarat
                                            </span>
                                            <span
                                              className="casino__provider-title"
                                              style={{ "font-size": "14px" }}
                                            >
                                              CREEDROOMZ
                                            </span>
                                          </div>
                                        </div>
                                        <div className="x-casinoGameCardDesktop__favoriteBtn">
                                          <span className="style__Container-sc-gq0272-0 iqWmzF">
                                            <span
                                              className="v3-icon "
                                              style={{
                                                "font-size": "16px",
                                                position: "relative",
                                              }}
                                            >
                                              <svg
                                                width="1em"
                                                height="1em"
                                                viewBox="0 0 32 32"
                                                fill="currentColor"
                                                xmlns="http://www.w3.org/2000/svg"
                                              >
                                                <path d="M22.8101 2.00043C21.4957 2.07648 20.2115 2.42514 19.0391 3.02424C17.8668 3.62335 16.8318 4.4598 16.0001 5.48043C15.1695 4.461 14.1362 3.62526 12.9656 3.0262C11.7951 2.42715 10.5128 2.07784 9.20013 2.00043C7.74063 1.9743 6.29765 2.31247 5.00173 2.98435C3.70582 3.65623 2.59787 4.64061 1.77813 5.84843C-4.40187 14.3484 7.29813 24.2904 10.0501 26.9604C11.6961 28.5604 13.7281 30.4544 14.9421 31.5784C15.2278 31.8492 15.6065 32.0002 16.0001 32.0002C16.3938 32.0002 16.7724 31.8492 17.0581 31.5784C18.2701 30.4544 20.3021 28.5584 21.9501 26.9604C24.7021 24.2904 36.4041 14.3484 30.2221 5.84843C29.4037 4.64165 28.2974 3.65787 27.0033 2.98602C25.7092 2.31417 24.268 1.97543 22.8101 2.00043V2.00043Z" />
                                              </svg>
                                            </span>
                                          </span>
                                        </div>
                                      </div>
                                    </div>
                                    <div
                                      className="x-casinoGameCardDesktop"
                                      data-testid="casino_game_card"
                                    >
                                      <div className="x-casinoGameCardImageWrapper">
                                        <img
                                          className="x-casinoGameCardImageWrapper__image"
                                          src="https://cmsbetconstruct.com/content/images/casino/icon2/ef580789659065d2e50120511a84a52c_casinoGameIcon2.webp?v=1686920992"
                                          alt="VBET Roulette"
                                          loading="lazy"
                                        />
                                        <div className="x-casinoGameCardDesktop__buttons">
                                          <div className="casino__real-btn">
                                            <div className="casino__real-btn-wrapper">
                                              <span
                                                className="v3-icon casino__real-btn-icon"
                                                style={{
                                                  "font-size": "44px",
                                                  position: "relative",
                                                }}
                                              >
                                                <svg
                                                  xmlns="http://www.w3.org/2000/svg"
                                                  width="1em"
                                                  height="1em"
                                                  viewBox="0 0 32 32"
                                                  fill="currentColor"
                                                >
                                                  <path d="M13.9013 23.4271C13.2464 23.4271 12.6213 23.2634 12.0706 22.9508C10.7906 22.2066 10.0762 20.748 10.0762 18.8429V13.8569C10.0762 11.9517 10.7757 10.4931 12.0557 9.74895C13.3357 9.00476 14.958 9.12383 16.6101 10.0764L20.9264 12.562C22.5785 13.5145 23.4864 14.8541 23.4864 16.3424C23.4864 17.8159 22.5785 19.1703 20.9264 20.1229L16.6101 22.6085C15.6873 23.1592 14.7497 23.4271 13.9013 23.4271ZM13.9013 11.4903C13.6334 11.4903 13.3804 11.5499 13.1869 11.6689C12.6213 11.9964 12.3087 12.7852 12.3087 13.8569V18.8429C12.3087 19.8996 12.6213 20.7034 13.1869 21.0159C13.7376 21.3434 14.5859 21.2094 15.5087 20.6885L19.825 18.2029C20.7478 17.6671 21.2687 16.9973 21.2687 16.3573C21.2687 15.7173 20.7329 15.0476 19.825 14.5117L15.5087 12.0262C14.9134 11.6689 14.3627 11.4903 13.9013 11.4903Z" />
                                                  <path d="M16 32C7.18884 32 0 24.826 0 16C0 7.17395 7.18884 0 16 0C24.8112 0 32 7.17395 32 16C32 24.826 24.826 32 16 32ZM16 2.23256C8.4093 2.23256 2.23256 8.4093 2.23256 16C2.23256 23.5907 8.4093 29.7674 16 29.7674C23.5907 29.7674 29.7674 23.5907 29.7674 16C29.7674 8.4093 23.5907 2.23256 16 2.23256Z" />
                                                </svg>
                                              </span>
                                              <span
                                                className="v3-icon casino__real-btn-icon-colored"
                                                style={{
                                                  "font-size": "44px",
                                                  position: "relative",
                                                }}
                                              >
                                                <svg
                                                  xmlns="http://www.w3.org/2000/svg"
                                                  width="1em"
                                                  height="1em"
                                                  viewBox="0 0 32 32"
                                                  fill="currentColor"
                                                >
                                                  <path d="M32 16C32 19.1645 31.0616 22.2579 29.3035 24.8891C27.5454 27.5203 25.0465 29.5711 22.1229 30.7821C19.1993 31.9931 15.9822 32.3099 12.8785 31.6926C9.77485 31.0752 6.92392 29.5513 4.68628 27.3137C2.44864 25.0761 0.924812 22.2251 0.307448 19.1214C-0.309915 16.0177 0.00691687 12.8007 1.21792 9.87706C2.42892 6.95344 4.47968 4.45459 7.11087 2.69648C9.74205 0.938383 12.8355 0 16 0C20.2426 0.00295546 24.3105 1.68961 27.3105 4.68955C30.3104 7.68949 31.997 11.7574 32 16Z" />
                                                  <path d="M23.2649 16.3677C23.2199 17.1494 22.9637 17.9045 22.5234 18.5533C22.0832 19.2021 20.7635 20.054 20.7635 20.054L16.5363 22.479C15.7382 22.9738 14.822 23.2479 13.8821 23.2728C13.2545 23.2724 12.6377 23.1114 12.0907 22.8052C11.7838 22.6243 11.5043 22.401 11.2606 22.1419C11.1394 22.0101 11.0297 21.8682 10.9328 21.7178C10.8254 21.5623 10.7305 21.3987 10.6489 21.2284C10.3962 20.6907 10.2399 20.2096 10.1901 19.5212C10.1711 19.2586 10.1901 19.1103 10.1901 18.847C10.1901 16.9488 10.1901 13.9863 10.1901 13.9863C10.1901 13.9863 10.1982 12.4035 10.5441 11.6908C10.89 10.978 11.4427 10.3849 12.1306 9.98806C12.8186 9.59118 13.6103 9.40877 14.4034 9.46437C15.1966 9.51998 16.58 10.3 16.58 10.3L20.8072 12.7249C21.5436 13.1246 22.1786 13.6862 22.6642 14.3669C22.9731 14.8037 23.1751 15.3065 23.254 15.8349C23.2743 16.0118 23.2779 16.1902 23.2649 16.3677Z" />
                                                </svg>
                                              </span>
                                            </div>
                                            <span
                                              className="casino__name"
                                              style={{ "font-size": "16px" }}
                                            >
                                              VBET Roulette
                                            </span>
                                            <span
                                              className="casino__provider-title"
                                              style={{ "font-size": "14px" }}
                                            >
                                              Vbet
                                            </span>
                                          </div>
                                        </div>
                                        <div className="x-casinoGameCardDesktop__favoriteBtn">
                                          <span className="style__Container-sc-gq0272-0 iqWmzF">
                                            <span
                                              className="v3-icon "
                                              style={{
                                                "font-size": "16px",
                                                position: "relative",
                                              }}
                                            >
                                              <svg
                                                width="1em"
                                                height="1em"
                                                viewBox="0 0 32 32"
                                                fill="currentColor"
                                                xmlns="http://www.w3.org/2000/svg"
                                              >
                                                <path d="M22.8101 2.00043C21.4957 2.07648 20.2115 2.42514 19.0391 3.02424C17.8668 3.62335 16.8318 4.4598 16.0001 5.48043C15.1695 4.461 14.1362 3.62526 12.9656 3.0262C11.7951 2.42715 10.5128 2.07784 9.20013 2.00043C7.74063 1.9743 6.29765 2.31247 5.00173 2.98435C3.70582 3.65623 2.59787 4.64061 1.77813 5.84843C-4.40187 14.3484 7.29813 24.2904 10.0501 26.9604C11.6961 28.5604 13.7281 30.4544 14.9421 31.5784C15.2278 31.8492 15.6065 32.0002 16.0001 32.0002C16.3938 32.0002 16.7724 31.8492 17.0581 31.5784C18.2701 30.4544 20.3021 28.5584 21.9501 26.9604C24.7021 24.2904 36.4041 14.3484 30.2221 5.84843C29.4037 4.64165 28.2974 3.65787 27.0033 2.98602C25.7092 2.31417 24.268 1.97543 22.8101 2.00043V2.00043Z" />
                                              </svg>
                                            </span>
                                          </span>
                                        </div>
                                      </div>
                                    </div>
                                    <div
                                      className="x-casinoGameCardDesktop"
                                      data-testid="casino_game_card"
                                    >
                                      <div className="x-casinoGameCardImageWrapper">
                                        <img
                                          className="x-casinoGameCardImageWrapper__image"
                                          src="https://cmsbetconstruct.com/content/images/casino/icon2/5f1645f16d7b9f50fff7fd51284c5108_casinoGameIcon2.gif?v=1686920992"
                                          alt="Express Roulette"
                                          loading="lazy"
                                        />
                                        <div className="x-casinoGameCardDesktop__buttons">
                                          <div className="casino__real-btn">
                                            <div className="casino__real-btn-wrapper">
                                              <span
                                                className="v3-icon casino__real-btn-icon"
                                                style={{
                                                  "font-size": "44px",
                                                  position: "relative",
                                                }}
                                              >
                                                <svg
                                                  xmlns="http://www.w3.org/2000/svg"
                                                  width="1em"
                                                  height="1em"
                                                  viewBox="0 0 32 32"
                                                  fill="currentColor"
                                                >
                                                  <path d="M13.9013 23.4271C13.2464 23.4271 12.6213 23.2634 12.0706 22.9508C10.7906 22.2066 10.0762 20.748 10.0762 18.8429V13.8569C10.0762 11.9517 10.7757 10.4931 12.0557 9.74895C13.3357 9.00476 14.958 9.12383 16.6101 10.0764L20.9264 12.562C22.5785 13.5145 23.4864 14.8541 23.4864 16.3424C23.4864 17.8159 22.5785 19.1703 20.9264 20.1229L16.6101 22.6085C15.6873 23.1592 14.7497 23.4271 13.9013 23.4271ZM13.9013 11.4903C13.6334 11.4903 13.3804 11.5499 13.1869 11.6689C12.6213 11.9964 12.3087 12.7852 12.3087 13.8569V18.8429C12.3087 19.8996 12.6213 20.7034 13.1869 21.0159C13.7376 21.3434 14.5859 21.2094 15.5087 20.6885L19.825 18.2029C20.7478 17.6671 21.2687 16.9973 21.2687 16.3573C21.2687 15.7173 20.7329 15.0476 19.825 14.5117L15.5087 12.0262C14.9134 11.6689 14.3627 11.4903 13.9013 11.4903Z" />
                                                  <path d="M16 32C7.18884 32 0 24.826 0 16C0 7.17395 7.18884 0 16 0C24.8112 0 32 7.17395 32 16C32 24.826 24.826 32 16 32ZM16 2.23256C8.4093 2.23256 2.23256 8.4093 2.23256 16C2.23256 23.5907 8.4093 29.7674 16 29.7674C23.5907 29.7674 29.7674 23.5907 29.7674 16C29.7674 8.4093 23.5907 2.23256 16 2.23256Z" />
                                                </svg>
                                              </span>
                                              <span
                                                className="v3-icon casino__real-btn-icon-colored"
                                                style={{
                                                  "font-size": "44px",
                                                  position: "relative",
                                                }}
                                              >
                                                <svg
                                                  xmlns="http://www.w3.org/2000/svg"
                                                  width="1em"
                                                  height="1em"
                                                  viewBox="0 0 32 32"
                                                  fill="currentColor"
                                                >
                                                  <path d="M32 16C32 19.1645 31.0616 22.2579 29.3035 24.8891C27.5454 27.5203 25.0465 29.5711 22.1229 30.7821C19.1993 31.9931 15.9822 32.3099 12.8785 31.6926C9.77485 31.0752 6.92392 29.5513 4.68628 27.3137C2.44864 25.0761 0.924812 22.2251 0.307448 19.1214C-0.309915 16.0177 0.00691687 12.8007 1.21792 9.87706C2.42892 6.95344 4.47968 4.45459 7.11087 2.69648C9.74205 0.938383 12.8355 0 16 0C20.2426 0.00295546 24.3105 1.68961 27.3105 4.68955C30.3104 7.68949 31.997 11.7574 32 16Z" />
                                                  <path d="M23.2649 16.3677C23.2199 17.1494 22.9637 17.9045 22.5234 18.5533C22.0832 19.2021 20.7635 20.054 20.7635 20.054L16.5363 22.479C15.7382 22.9738 14.822 23.2479 13.8821 23.2728C13.2545 23.2724 12.6377 23.1114 12.0907 22.8052C11.7838 22.6243 11.5043 22.401 11.2606 22.1419C11.1394 22.0101 11.0297 21.8682 10.9328 21.7178C10.8254 21.5623 10.7305 21.3987 10.6489 21.2284C10.3962 20.6907 10.2399 20.2096 10.1901 19.5212C10.1711 19.2586 10.1901 19.1103 10.1901 18.847C10.1901 16.9488 10.1901 13.9863 10.1901 13.9863C10.1901 13.9863 10.1982 12.4035 10.5441 11.6908C10.89 10.978 11.4427 10.3849 12.1306 9.98806C12.8186 9.59118 13.6103 9.40877 14.4034 9.46437C15.1966 9.51998 16.58 10.3 16.58 10.3L20.8072 12.7249C21.5436 13.1246 22.1786 13.6862 22.6642 14.3669C22.9731 14.8037 23.1751 15.3065 23.254 15.8349C23.2743 16.0118 23.2779 16.1902 23.2649 16.3677Z" />
                                                </svg>
                                              </span>
                                            </div>
                                            <span
                                              className="casino__name"
                                              style={{ "font-size": "16px" }}
                                            >
                                              Express Roulette
                                            </span>
                                            <span
                                              className="casino__provider-title"
                                              style={{ "font-size": "14px" }}
                                            >
                                              CREEDROOMZ
                                            </span>
                                          </div>
                                        </div>
                                        <div className="x-casinoGameCardDesktop__favoriteBtn">
                                          <span className="style__Container-sc-gq0272-0 iqWmzF">
                                            <span
                                              className="v3-icon "
                                              style={{
                                                "font-size": "16px",
                                                position: "relative",
                                              }}
                                            >
                                              <svg
                                                width="1em"
                                                height="1em"
                                                viewBox="0 0 32 32"
                                                fill="currentColor"
                                                xmlns="http://www.w3.org/2000/svg"
                                              >
                                                <path d="M22.8101 2.00043C21.4957 2.07648 20.2115 2.42514 19.0391 3.02424C17.8668 3.62335 16.8318 4.4598 16.0001 5.48043C15.1695 4.461 14.1362 3.62526 12.9656 3.0262C11.7951 2.42715 10.5128 2.07784 9.20013 2.00043C7.74063 1.9743 6.29765 2.31247 5.00173 2.98435C3.70582 3.65623 2.59787 4.64061 1.77813 5.84843C-4.40187 14.3484 7.29813 24.2904 10.0501 26.9604C11.6961 28.5604 13.7281 30.4544 14.9421 31.5784C15.2278 31.8492 15.6065 32.0002 16.0001 32.0002C16.3938 32.0002 16.7724 31.8492 17.0581 31.5784C18.2701 30.4544 20.3021 28.5584 21.9501 26.9604C24.7021 24.2904 36.4041 14.3484 30.2221 5.84843C29.4037 4.64165 28.2974 3.65787 27.0033 2.98602C25.7092 2.31417 24.268 1.97543 22.8101 2.00043V2.00043Z" />
                                              </svg>
                                            </span>
                                          </span>
                                        </div>
                                      </div>
                                    </div>
                                    <div
                                      className="x-casinoGameCardDesktop"
                                      data-testid="casino_game_card"
                                    >
                                      <div className="x-casinoGameCardImageWrapper">
                                        <img
                                          className="x-casinoGameCardImageWrapper__image"
                                          src="https://cmsbetconstruct.com/content/images/casino/icon2/7aa2aa5944324810c67437a21238e964_casinoGameIcon2.webp?v=1686920992"
                                          alt="Russian Poker"
                                          loading="lazy"
                                        />
                                        <div className="x-casinoGameCardDesktop__buttons">
                                          <div className="casino__real-btn">
                                            <div className="casino__real-btn-wrapper">
                                              <span
                                                className="v3-icon casino__real-btn-icon"
                                                style={{
                                                  "font-size": "44px",
                                                  position: "relative",
                                                }}
                                              >
                                                <svg
                                                  xmlns="http://www.w3.org/2000/svg"
                                                  width="1em"
                                                  height="1em"
                                                  viewBox="0 0 32 32"
                                                  fill="currentColor"
                                                >
                                                  <path d="M13.9013 23.4271C13.2464 23.4271 12.6213 23.2634 12.0706 22.9508C10.7906 22.2066 10.0762 20.748 10.0762 18.8429V13.8569C10.0762 11.9517 10.7757 10.4931 12.0557 9.74895C13.3357 9.00476 14.958 9.12383 16.6101 10.0764L20.9264 12.562C22.5785 13.5145 23.4864 14.8541 23.4864 16.3424C23.4864 17.8159 22.5785 19.1703 20.9264 20.1229L16.6101 22.6085C15.6873 23.1592 14.7497 23.4271 13.9013 23.4271ZM13.9013 11.4903C13.6334 11.4903 13.3804 11.5499 13.1869 11.6689C12.6213 11.9964 12.3087 12.7852 12.3087 13.8569V18.8429C12.3087 19.8996 12.6213 20.7034 13.1869 21.0159C13.7376 21.3434 14.5859 21.2094 15.5087 20.6885L19.825 18.2029C20.7478 17.6671 21.2687 16.9973 21.2687 16.3573C21.2687 15.7173 20.7329 15.0476 19.825 14.5117L15.5087 12.0262C14.9134 11.6689 14.3627 11.4903 13.9013 11.4903Z" />
                                                  <path d="M16 32C7.18884 32 0 24.826 0 16C0 7.17395 7.18884 0 16 0C24.8112 0 32 7.17395 32 16C32 24.826 24.826 32 16 32ZM16 2.23256C8.4093 2.23256 2.23256 8.4093 2.23256 16C2.23256 23.5907 8.4093 29.7674 16 29.7674C23.5907 29.7674 29.7674 23.5907 29.7674 16C29.7674 8.4093 23.5907 2.23256 16 2.23256Z" />
                                                </svg>
                                              </span>
                                              <span
                                                className="v3-icon casino__real-btn-icon-colored"
                                                style={{
                                                  "font-size": "44px",
                                                  position: "relative",
                                                }}
                                              >
                                                <svg
                                                  xmlns="http://www.w3.org/2000/svg"
                                                  width="1em"
                                                  height="1em"
                                                  viewBox="0 0 32 32"
                                                  fill="currentColor"
                                                >
                                                  <path d="M32 16C32 19.1645 31.0616 22.2579 29.3035 24.8891C27.5454 27.5203 25.0465 29.5711 22.1229 30.7821C19.1993 31.9931 15.9822 32.3099 12.8785 31.6926C9.77485 31.0752 6.92392 29.5513 4.68628 27.3137C2.44864 25.0761 0.924812 22.2251 0.307448 19.1214C-0.309915 16.0177 0.00691687 12.8007 1.21792 9.87706C2.42892 6.95344 4.47968 4.45459 7.11087 2.69648C9.74205 0.938383 12.8355 0 16 0C20.2426 0.00295546 24.3105 1.68961 27.3105 4.68955C30.3104 7.68949 31.997 11.7574 32 16Z" />
                                                  <path d="M23.2649 16.3677C23.2199 17.1494 22.9637 17.9045 22.5234 18.5533C22.0832 19.2021 20.7635 20.054 20.7635 20.054L16.5363 22.479C15.7382 22.9738 14.822 23.2479 13.8821 23.2728C13.2545 23.2724 12.6377 23.1114 12.0907 22.8052C11.7838 22.6243 11.5043 22.401 11.2606 22.1419C11.1394 22.0101 11.0297 21.8682 10.9328 21.7178C10.8254 21.5623 10.7305 21.3987 10.6489 21.2284C10.3962 20.6907 10.2399 20.2096 10.1901 19.5212C10.1711 19.2586 10.1901 19.1103 10.1901 18.847C10.1901 16.9488 10.1901 13.9863 10.1901 13.9863C10.1901 13.9863 10.1982 12.4035 10.5441 11.6908C10.89 10.978 11.4427 10.3849 12.1306 9.98806C12.8186 9.59118 13.6103 9.40877 14.4034 9.46437C15.1966 9.51998 16.58 10.3 16.58 10.3L20.8072 12.7249C21.5436 13.1246 22.1786 13.6862 22.6642 14.3669C22.9731 14.8037 23.1751 15.3065 23.254 15.8349C23.2743 16.0118 23.2779 16.1902 23.2649 16.3677Z" />
                                                </svg>
                                              </span>
                                            </div>
                                            <span
                                              className="casino__name"
                                              style={{ "font-size": "16px" }}
                                            >
                                              Russian Poker
                                            </span>
                                            <span
                                              className="casino__provider-title"
                                              style={{ "font-size": "14px" }}
                                            >
                                              Live Vegas
                                            </span>
                                          </div>
                                        </div>
                                        <div className="x-casinoGameCardDesktop__favoriteBtn">
                                          <span className="style__Container-sc-gq0272-0 iqWmzF">
                                            <span
                                              className="v3-icon "
                                              style={{
                                                "font-size": "16px",
                                                position: "relative",
                                              }}
                                            >
                                              <svg
                                                width="1em"
                                                height="1em"
                                                viewBox="0 0 32 32"
                                                fill="currentColor"
                                                xmlns="http://www.w3.org/2000/svg"
                                              >
                                                <path d="M22.8101 2.00043C21.4957 2.07648 20.2115 2.42514 19.0391 3.02424C17.8668 3.62335 16.8318 4.4598 16.0001 5.48043C15.1695 4.461 14.1362 3.62526 12.9656 3.0262C11.7951 2.42715 10.5128 2.07784 9.20013 2.00043C7.74063 1.9743 6.29765 2.31247 5.00173 2.98435C3.70582 3.65623 2.59787 4.64061 1.77813 5.84843C-4.40187 14.3484 7.29813 24.2904 10.0501 26.9604C11.6961 28.5604 13.7281 30.4544 14.9421 31.5784C15.2278 31.8492 15.6065 32.0002 16.0001 32.0002C16.3938 32.0002 16.7724 31.8492 17.0581 31.5784C18.2701 30.4544 20.3021 28.5584 21.9501 26.9604C24.7021 24.2904 36.4041 14.3484 30.2221 5.84843C29.4037 4.64165 28.2974 3.65787 27.0033 2.98602C25.7092 2.31417 24.268 1.97543 22.8101 2.00043V2.00043Z" />
                                              </svg>
                                            </span>
                                          </span>
                                        </div>
                                      </div>
                                    </div>
                                    <div
                                      className="x-casinoGameCardDesktop"
                                      data-testid="casino_game_card"
                                    >
                                      <div className="x-casinoGameCardImageWrapper">
                                        <img
                                          className="x-casinoGameCardImageWrapper__image"
                                          src="https://cmsbetconstruct.com/content/images/casino/icon2/e1e4eaa882731befcdd66ccee3f88396_casinoGameIcon2.webp?v=1686920992"
                                          alt="Atrium Private Blackjack"
                                          loading="lazy"
                                        />
                                        <div className="x-casinoGameCardDesktop__buttons">
                                          <div className="casino__real-btn">
                                            <div className="casino__real-btn-wrapper">
                                              <span
                                                className="v3-icon casino__real-btn-icon"
                                                style={{
                                                  "font-size": "44px",
                                                  position: "relative",
                                                }}
                                              >
                                                <svg
                                                  xmlns="http://www.w3.org/2000/svg"
                                                  width="1em"
                                                  height="1em"
                                                  viewBox="0 0 32 32"
                                                  fill="currentColor"
                                                >
                                                  <path d="M13.9013 23.4271C13.2464 23.4271 12.6213 23.2634 12.0706 22.9508C10.7906 22.2066 10.0762 20.748 10.0762 18.8429V13.8569C10.0762 11.9517 10.7757 10.4931 12.0557 9.74895C13.3357 9.00476 14.958 9.12383 16.6101 10.0764L20.9264 12.562C22.5785 13.5145 23.4864 14.8541 23.4864 16.3424C23.4864 17.8159 22.5785 19.1703 20.9264 20.1229L16.6101 22.6085C15.6873 23.1592 14.7497 23.4271 13.9013 23.4271ZM13.9013 11.4903C13.6334 11.4903 13.3804 11.5499 13.1869 11.6689C12.6213 11.9964 12.3087 12.7852 12.3087 13.8569V18.8429C12.3087 19.8996 12.6213 20.7034 13.1869 21.0159C13.7376 21.3434 14.5859 21.2094 15.5087 20.6885L19.825 18.2029C20.7478 17.6671 21.2687 16.9973 21.2687 16.3573C21.2687 15.7173 20.7329 15.0476 19.825 14.5117L15.5087 12.0262C14.9134 11.6689 14.3627 11.4903 13.9013 11.4903Z" />
                                                  <path d="M16 32C7.18884 32 0 24.826 0 16C0 7.17395 7.18884 0 16 0C24.8112 0 32 7.17395 32 16C32 24.826 24.826 32 16 32ZM16 2.23256C8.4093 2.23256 2.23256 8.4093 2.23256 16C2.23256 23.5907 8.4093 29.7674 16 29.7674C23.5907 29.7674 29.7674 23.5907 29.7674 16C29.7674 8.4093 23.5907 2.23256 16 2.23256Z" />
                                                </svg>
                                              </span>
                                              <span
                                                className="v3-icon casino__real-btn-icon-colored"
                                                style={{
                                                  "font-size": "44px",
                                                  position: "relative",
                                                }}
                                              >
                                                <svg
                                                  xmlns="http://www.w3.org/2000/svg"
                                                  width="1em"
                                                  height="1em"
                                                  viewBox="0 0 32 32"
                                                  fill="currentColor"
                                                >
                                                  <path d="M32 16C32 19.1645 31.0616 22.2579 29.3035 24.8891C27.5454 27.5203 25.0465 29.5711 22.1229 30.7821C19.1993 31.9931 15.9822 32.3099 12.8785 31.6926C9.77485 31.0752 6.92392 29.5513 4.68628 27.3137C2.44864 25.0761 0.924812 22.2251 0.307448 19.1214C-0.309915 16.0177 0.00691687 12.8007 1.21792 9.87706C2.42892 6.95344 4.47968 4.45459 7.11087 2.69648C9.74205 0.938383 12.8355 0 16 0C20.2426 0.00295546 24.3105 1.68961 27.3105 4.68955C30.3104 7.68949 31.997 11.7574 32 16Z" />
                                                  <path d="M23.2649 16.3677C23.2199 17.1494 22.9637 17.9045 22.5234 18.5533C22.0832 19.2021 20.7635 20.054 20.7635 20.054L16.5363 22.479C15.7382 22.9738 14.822 23.2479 13.8821 23.2728C13.2545 23.2724 12.6377 23.1114 12.0907 22.8052C11.7838 22.6243 11.5043 22.401 11.2606 22.1419C11.1394 22.0101 11.0297 21.8682 10.9328 21.7178C10.8254 21.5623 10.7305 21.3987 10.6489 21.2284C10.3962 20.6907 10.2399 20.2096 10.1901 19.5212C10.1711 19.2586 10.1901 19.1103 10.1901 18.847C10.1901 16.9488 10.1901 13.9863 10.1901 13.9863C10.1901 13.9863 10.1982 12.4035 10.5441 11.6908C10.89 10.978 11.4427 10.3849 12.1306 9.98806C12.8186 9.59118 13.6103 9.40877 14.4034 9.46437C15.1966 9.51998 16.58 10.3 16.58 10.3L20.8072 12.7249C21.5436 13.1246 22.1786 13.6862 22.6642 14.3669C22.9731 14.8037 23.1751 15.3065 23.254 15.8349C23.2743 16.0118 23.2779 16.1902 23.2649 16.3677Z" />
                                                </svg>
                                              </span>
                                            </div>
                                            <span
                                              className="casino__name"
                                              style={{ "font-size": "16px" }}
                                            >
                                              Atrium Private Blackjack
                                            </span>
                                            <span
                                              className="casino__provider-title"
                                              style={{ "font-size": "14px" }}
                                            >
                                              CREEDROOMZ
                                            </span>
                                          </div>
                                        </div>
                                        <div className="x-casinoGameCardDesktop__favoriteBtn">
                                          <span className="style__Container-sc-gq0272-0 iqWmzF">
                                            <span
                                              className="v3-icon "
                                              style={{
                                                "font-size": "16px",
                                                position: "relative",
                                              }}
                                            >
                                              <svg
                                                width="1em"
                                                height="1em"
                                                viewBox="0 0 32 32"
                                                fill="currentColor"
                                                xmlns="http://www.w3.org/2000/svg"
                                              >
                                                <path d="M22.8101 2.00043C21.4957 2.07648 20.2115 2.42514 19.0391 3.02424C17.8668 3.62335 16.8318 4.4598 16.0001 5.48043C15.1695 4.461 14.1362 3.62526 12.9656 3.0262C11.7951 2.42715 10.5128 2.07784 9.20013 2.00043C7.74063 1.9743 6.29765 2.31247 5.00173 2.98435C3.70582 3.65623 2.59787 4.64061 1.77813 5.84843C-4.40187 14.3484 7.29813 24.2904 10.0501 26.9604C11.6961 28.5604 13.7281 30.4544 14.9421 31.5784C15.2278 31.8492 15.6065 32.0002 16.0001 32.0002C16.3938 32.0002 16.7724 31.8492 17.0581 31.5784C18.2701 30.4544 20.3021 28.5584 21.9501 26.9604C24.7021 24.2904 36.4041 14.3484 30.2221 5.84843C29.4037 4.64165 28.2974 3.65787 27.0033 2.98602C25.7092 2.31417 24.268 1.97543 22.8101 2.00043V2.00043Z" />
                                              </svg>
                                            </span>
                                          </span>
                                        </div>
                                      </div>
                                    </div>
                                    <div
                                      className="x-casinoGameCardDesktop"
                                      data-testid="casino_game_card"
                                    >
                                      <div className="x-casinoGameCardImageWrapper">
                                        <img
                                          className="x-casinoGameCardImageWrapper__image"
                                          src="https://cmsbetconstruct.com/content/images/casino/icon2/aae468833f993aefa1b67e79ef9692eb_casinoGameIcon2.webp?v=1686920992"
                                          alt="Roba Baccarat No Commission"
                                          loading="lazy"
                                        />
                                        <div className="x-casinoGameCardDesktop__buttons">
                                          <div className="casino__real-btn">
                                            <div className="casino__real-btn-wrapper">
                                              <span
                                                className="v3-icon casino__real-btn-icon"
                                                style={{
                                                  "font-size": "44px",
                                                  position: "relative",
                                                }}
                                              >
                                                <svg
                                                  xmlns="http://www.w3.org/2000/svg"
                                                  width="1em"
                                                  height="1em"
                                                  viewBox="0 0 32 32"
                                                  fill="currentColor"
                                                >
                                                  <path d="M13.9013 23.4271C13.2464 23.4271 12.6213 23.2634 12.0706 22.9508C10.7906 22.2066 10.0762 20.748 10.0762 18.8429V13.8569C10.0762 11.9517 10.7757 10.4931 12.0557 9.74895C13.3357 9.00476 14.958 9.12383 16.6101 10.0764L20.9264 12.562C22.5785 13.5145 23.4864 14.8541 23.4864 16.3424C23.4864 17.8159 22.5785 19.1703 20.9264 20.1229L16.6101 22.6085C15.6873 23.1592 14.7497 23.4271 13.9013 23.4271ZM13.9013 11.4903C13.6334 11.4903 13.3804 11.5499 13.1869 11.6689C12.6213 11.9964 12.3087 12.7852 12.3087 13.8569V18.8429C12.3087 19.8996 12.6213 20.7034 13.1869 21.0159C13.7376 21.3434 14.5859 21.2094 15.5087 20.6885L19.825 18.2029C20.7478 17.6671 21.2687 16.9973 21.2687 16.3573C21.2687 15.7173 20.7329 15.0476 19.825 14.5117L15.5087 12.0262C14.9134 11.6689 14.3627 11.4903 13.9013 11.4903Z" />
                                                  <path d="M16 32C7.18884 32 0 24.826 0 16C0 7.17395 7.18884 0 16 0C24.8112 0 32 7.17395 32 16C32 24.826 24.826 32 16 32ZM16 2.23256C8.4093 2.23256 2.23256 8.4093 2.23256 16C2.23256 23.5907 8.4093 29.7674 16 29.7674C23.5907 29.7674 29.7674 23.5907 29.7674 16C29.7674 8.4093 23.5907 2.23256 16 2.23256Z" />
                                                </svg>
                                              </span>
                                              <span
                                                className="v3-icon casino__real-btn-icon-colored"
                                                style={{
                                                  "font-size": "44px",
                                                  position: "relative",
                                                }}
                                              >
                                                <svg
                                                  xmlns="http://www.w3.org/2000/svg"
                                                  width="1em"
                                                  height="1em"
                                                  viewBox="0 0 32 32"
                                                  fill="currentColor"
                                                >
                                                  <path d="M32 16C32 19.1645 31.0616 22.2579 29.3035 24.8891C27.5454 27.5203 25.0465 29.5711 22.1229 30.7821C19.1993 31.9931 15.9822 32.3099 12.8785 31.6926C9.77485 31.0752 6.92392 29.5513 4.68628 27.3137C2.44864 25.0761 0.924812 22.2251 0.307448 19.1214C-0.309915 16.0177 0.00691687 12.8007 1.21792 9.87706C2.42892 6.95344 4.47968 4.45459 7.11087 2.69648C9.74205 0.938383 12.8355 0 16 0C20.2426 0.00295546 24.3105 1.68961 27.3105 4.68955C30.3104 7.68949 31.997 11.7574 32 16Z" />
                                                  <path d="M23.2649 16.3677C23.2199 17.1494 22.9637 17.9045 22.5234 18.5533C22.0832 19.2021 20.7635 20.054 20.7635 20.054L16.5363 22.479C15.7382 22.9738 14.822 23.2479 13.8821 23.2728C13.2545 23.2724 12.6377 23.1114 12.0907 22.8052C11.7838 22.6243 11.5043 22.401 11.2606 22.1419C11.1394 22.0101 11.0297 21.8682 10.9328 21.7178C10.8254 21.5623 10.7305 21.3987 10.6489 21.2284C10.3962 20.6907 10.2399 20.2096 10.1901 19.5212C10.1711 19.2586 10.1901 19.1103 10.1901 18.847C10.1901 16.9488 10.1901 13.9863 10.1901 13.9863C10.1901 13.9863 10.1982 12.4035 10.5441 11.6908C10.89 10.978 11.4427 10.3849 12.1306 9.98806C12.8186 9.59118 13.6103 9.40877 14.4034 9.46437C15.1966 9.51998 16.58 10.3 16.58 10.3L20.8072 12.7249C21.5436 13.1246 22.1786 13.6862 22.6642 14.3669C22.9731 14.8037 23.1751 15.3065 23.254 15.8349C23.2743 16.0118 23.2779 16.1902 23.2649 16.3677Z" />
                                                </svg>
                                              </span>
                                            </div>
                                            <span
                                              className="casino__name"
                                              style={{ "font-size": "16px" }}
                                            >
                                              Roba Baccarat No Commission
                                            </span>
                                            <span
                                              className="casino__provider-title"
                                              style={{ "font-size": "14px" }}
                                            >
                                              CREEDROOMZ
                                            </span>
                                          </div>
                                        </div>
                                        <div className="x-casinoGameCardDesktop__favoriteBtn">
                                          <span className="style__Container-sc-gq0272-0 iqWmzF">
                                            <span
                                              className="v3-icon "
                                              style={{
                                                "font-size": "16px",
                                                position: "relative",
                                              }}
                                            >
                                              <svg
                                                width="1em"
                                                height="1em"
                                                viewBox="0 0 32 32"
                                                fill="currentColor"
                                                xmlns="http://www.w3.org/2000/svg"
                                              >
                                                <path d="M22.8101 2.00043C21.4957 2.07648 20.2115 2.42514 19.0391 3.02424C17.8668 3.62335 16.8318 4.4598 16.0001 5.48043C15.1695 4.461 14.1362 3.62526 12.9656 3.0262C11.7951 2.42715 10.5128 2.07784 9.20013 2.00043C7.74063 1.9743 6.29765 2.31247 5.00173 2.98435C3.70582 3.65623 2.59787 4.64061 1.77813 5.84843C-4.40187 14.3484 7.29813 24.2904 10.0501 26.9604C11.6961 28.5604 13.7281 30.4544 14.9421 31.5784C15.2278 31.8492 15.6065 32.0002 16.0001 32.0002C16.3938 32.0002 16.7724 31.8492 17.0581 31.5784C18.2701 30.4544 20.3021 28.5584 21.9501 26.9604C24.7021 24.2904 36.4041 14.3484 30.2221 5.84843C29.4037 4.64165 28.2974 3.65787 27.0033 2.98602C25.7092 2.31417 24.268 1.97543 22.8101 2.00043V2.00043Z" />
                                              </svg>
                                            </span>
                                          </span>
                                        </div>
                                      </div>
                                    </div>
                                  </div>
                                </div>
                                <button
                                  type="button"
                                  className="v3-btn v3-btn-ghost v3-btn-lg x-button x-casinoGames__loadMoreButton x-casinoGames__loadMoreButton--desktop"
                                >
                                  <span>Load more</span>
                                </button>
                              </div>
                            </div>
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </main>
    </>
  );
};

export default LivecasinoPage;
